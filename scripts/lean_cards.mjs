...
if (CHECK) {
  ...
  if (publicLoaded.raw !== output) {
    errors.push("public 직렬화가 canonical generated bytes와 불일치");
  }
  if (errors.length) {
  ...
  console.log(`PASS: public ${publicLoaded.doc.cards.length}장 ≡ canonical full 정확 사영 (KEEP ${KEEP.length})`);
  process.exit(0);
}
...
const unchanged = currentRaw !== null && currentRaw === output;
...
if (unchanged) {
  console.log("변경 없음 — public projection의 값과 직렬화가 canonical output과 일치");
  process.exit(0);
}
if (DRY) {
...
const written = load(LEAN_PATH);
const errors = validateProjection(written.doc);
if (written.raw !== output) {
  errors.push("생성 후 public 직렬화 불일치");
}
if (errors.length) {
  errors.slice(0, 8).forEach((error) => console.error("FAIL:", error));
...
console.log(`완료: ${FULL_PATH}은 변경하지 않고 ${LEAN_PATH}만 정확 직렬화로 재생성`);
