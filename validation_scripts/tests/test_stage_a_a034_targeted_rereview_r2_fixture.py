...
class TestStageAA034TargetedRereviewR2(unittest.TestCase):
    ...
    def test_01_targeted_accounting_and_watchlist_routing(self):
        ...
        self.assertIn("no Stage B normal draft", item["recommended_next_action"])
    ...
    def test_02_stage_a_boundary_no_fetch_no_card_copy(self):
        ...
        b = d["boundary"]
        self.assertFalse(b["external_web_search_performed"])
        self.assertFalse(b["article_body_fetch_performed"])
    ...
    def test_04_csv_contract_and_report_boundary(self):
        ...
        row = rows[0]
        required = {
            "story_id","region","original_triage_status","status_detail","stage_a_bucket","ledger_decision",
            "headline","reason","baseline_relation","duplicate_risk","staleness_decision","event_date",
            "source_tier_estimate","source_access_risk","format_risk_tags","execution_anchor_type",
            "execution_anchor_strength","structural_value_override_applied","anchor_classes","evidence_needed_for_stage_b",
            "why_execution_event_not_required","strict_pass_gate_status","strict_pass_gate_reason","review_pool_partition",
            "review_pool_subtype","review_pool_partition_reason","promotion_precondition","bounded_review_question",
