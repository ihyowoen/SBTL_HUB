...
export async function loadKnowledge() {
  const [cardsRes, faqRes, trackerRes] = await Promise.all([
    loadOne("cards.json"),
    loadOne("faq.json"),
  ...
  // 진단 로그 — source 분포 확인용
  console.log(
    `[loadKnowledge] cards=${cards.length}(${cardsRes.source}) faq=${faq.length}(${faqRes.source}) tracker=${tracker.items.length}(${trackerRes.source})`
  );

  return { cards, faq, tracker, _sources: { cards: cardsRes.source, faq: faqRes.source, tracker: trackerRes.source } };
}
