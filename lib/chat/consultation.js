...
function extractValueLike(text) {
  if (!text) return null;
  const re = /([0-9][0-9,.]*)\s*(GWh|MWh|kWh|GW|MW|kt|kg|t|%|배|만\s*대|만\s*달러|억\s*달러|조\s*달러|억\s*원|조\s*원|억|조|만|달러)/;
...
function buildScoutFallback({ card }) {
  ...
  // 각 fact는 stage2 LLM의 입력이 됨. action에 placeholder 문자열을 넣으면
  // stage2가 의미 있는 해석을 만들 수 없으므로 action은 null로 두고,
  // time(=card.date) 또는 place(=card.region)로 validator의
  // "최소 1필드 채워짐" 조건을 만족시킨다. value는 raw에서 수치 한 토막을
  // 뽑아낼 수 있으면 채운다.
  const sharedTime = card.date || null;
  const sharedPlace = card.region || null;

  // f1: card.fact
  if (card.fact && String(card.fact).trim()) {
    const raw = String(card.fact).trim();
    facts.push({
      id: `f${nextId++}`,
      subject,
      action: null,
      value: extractValueLike(raw),
      time: sharedTime,
      place: sharedPlace,
      raw: clampLen(raw, 15, 110),
      source: 'card_fact',
    });
  }

  // f2~: card.implication 배열
  if (Array.isArray(card.implication)) {
    for (const imp of card.implication) {
      if (!imp || !String(imp).trim()) continue;
      ...
      facts.push({
        ...
        value: extractValueLike(raw),
        time: sharedTime,
        place: null,  // implication은 보통 장소 정보 없음
        raw: clampLen(raw, 15, 110),
  ...
  // 최소 1개 보장 (card.fact도 implication도 없는 카드)
  if (facts.length === 0) {
  ...
  // Safety net — 모든 fillable 필드가 비어있으면 validator 위반.
  // card.date·card.region 둘 다 없는 극단 케이스에서만 발동.
  for (const f of facts) {
    if (!f.action && !f.value && !f.time && !f.place) {
      f.place = subject; // 최후 fallback: subject를 place로 (전치사적 사용 아님, 구분자 역할)
    }
  }

  // summary: card.gate 우선, 없으면 sub, 그 다음 title.
  // validator 30~250자 통과만 책임진다. 자기 폭로 문구 없음.
  let summary = String(card.gate || card.sub || '').trim();
  if (!summary) summary = String(card.title || '').trim();
  if (summary.length < 30) {
    // gate·sub·title 다 짧으면 title + sub로 보강
    const tail = String(card.sub || card.title || '').trim();
    if (tail && tail !== summary) summary = `${summary} ${tail}`.trim();
  }
  // 그래도 30자 미만이면 정직하게 짧게 (clampLen이 30자 미만은 padding 없이 그대로 둠)
  summary = clampLen(summary, 30, 240);
  // padding 없이 30자 미만으로 끝나면 validator 위반 — 마지막 안전망
  if (summary.length < 30) {
    summary = `${summary} 카드가 접수됐어`.trim();
    summary = clampLen(summary, 30, 240);
  }

  // unknowns: 카드 자체 정보 부족 영역만 짧게 표기. self-disclosure 금지.
  // 빈 배열이 가장 깔끔하지만 stage2 unknowns 처리 규칙을 활용하려면
  // 카드 데이터 기반 실제 gap을 표기하는 게 좋음. 보수적으로 비워둠.
  return {
    summary,
    facts,
    unknowns: [],
...
function buildAnalystFallback({ stage1Facts, card }) {
  // angle 자동 결정 (decision tree 모방).
  // 주의: fallback Scout이 모든 fact에 동일 card.date를 채우면 timeCount가
  // trivially facts.length가 되어 "타이밍" 단조 선택됨. 그래서 unique time
  // 값 개수로 판단해야 의미 있는 분기가 됨.
  ...
  const hasStructuralVerb = facts.some(f =>
    /지분|계약|거래|공급|인수|합병|JV|합작/.test(String(f.action || '') + String(f.raw || ''))
  );

  let angle = '대비';
  if (uniqueTimes.size >= 2) angle = '타이밍';
  else if (valueCount >= 1) angle = '숫자';
  else if (hasStructuralVerb) angle = '구조';

  // fact id 참조 (validator: ≥2개 또는 facts==1이면 ≥1)
  // 가운데점(·) 구분으로 받침 무관 자연 결합. 조사는 마지막 글자 숫자 가정 → "를"
  const ids = facts.map(f => f.id);
  const refStr = ids.length >= 2 ? `${ids[0]}·${ids[1]}` : (ids[0] || 'f1');

  // interpretation 본문 — 카드 함의 한 줄을 fact id 묶음 안에 자연스럽게 인용.
  // 자기 폭로 문구 없음. "강차장이 한 각도로 잡았다"는 톤만 유지.
  const cardImpFirst = (Array.isArray(card.implication) && card.implication[0]) || card.gate || '';
  const cardImpFirstTrim = clampLen(String(cardImpFirst), 0, 130);

  let interpretation;
  if (ids.length >= 2) {
    interpretation = `걸리는 건 ${angle} 쪽이야. ${refStr}를 묶어보면 ${cardImpFirstTrim} 이게 핵심으로 잡혀. 다른 각도들은 facts 안에서 충분히 안 갈라져서 일단 ${angle} 한 축으로 좁혀봤어.`;
  } else {
    interpretation = `걸리는 건 ${angle} 쪽이야. ${refStr} 하나로만 봐도 ${cardImpFirstTrim} 이게 핵심으로 잡혀. 다만 fact 수가 적어서 다른 각도는 별도 확인이 필요해.`;
  }
  interpretation = clampLen(interpretation, 60, 380);

  // key_tension: card.gate 또는 함의 첫 줄 압축 — padding 텍스트 제거
  let keyTension = String(card.gate || cardImpFirstTrim || '').split(/[.。]/)[0].trim();
  // 8자 미만이면 보조 정보로 보강 (자기 폭로 padding 사용 안 함)
  if (keyTension.length < 8) {
    const fallback = `${pickSubject(card)} ${angle} 각도`;
    keyTension = fallback;
  }
  keyTension = clampLen(keyTension, 8, 70);

  return { angle, interpretation, key_tension: keyTension };
}

function buildRedTeamFallback({ stage1Facts, stage2, card }) {
  const facts = Array.isArray(stage1Facts) ? stage1Facts : [];
  const ids = facts.map(f => f.id);
  // 가운데점(·) 구분 — 받침 무관, 조사는 "를/가" (숫자 끝 받침 없음 가정)
  const refStr = ids.length >= 2 ? `${ids[0]}·${ids[1]}` : (ids[0] || 'f1');

  const angle = (stage2 && stage2.angle) || '대비';

  // premature_interpretations — validator: 각 항목에 "stage2" 또는 "2차" 키워드 필수
  const premature = `stage2가 ${angle} 각도 하나로 잡았어. ${refStr}를 같은 묶음으로 본다는 전제가 facts 안에서 직접 검증되진 않은 자리거든`;

  // unverified_premises — stage2 전제를 명시적으로 짚기 (자기 폭로 없음)
  const unverified = `'${refStr}가 같은 방향 신호'라는 stage2 전제 — facts 안에서 다른 각도 가능성과 직접 비교되진 않음`;

  // counter_scenario — validator: 40~320자, fact id 참조 ≥1개
  // keyTension을 본문에 직접 인용하면 길이 cap에 걸렸을 때 조사가 어색해짐.
  // 일반화 표현으로 안정성 우선.
  const counterScenario =
    `${refStr}를 같은 묶음이 아니라 별개 사건으로 읽으면 ${angle} 각도는 약해져. ` +
    `이 경우 stage2가 잡은 결론은 일시적 현상으로 좁혀질 수도 있고, ` +
    `결론을 한 각도로 굳히기 전에 두 fact가 실제 한 방향인지 별도 확인이 필요한 자리야.`;

  // next_checkpoints — 카테고리별 검증 가능한 관측치 2개. 자기 폭로 금지.
  const checkpoints = checkpointsByCategory(card);

  return {
    premature_interpretations: [premature],
    unverified_premises: [unverified],
    counter_scenario: clampLen(counterScenario, 50, 310),
...
function makeScoutFallbackResult({ cardContext, latencyMs, provider, fallback_from, reason }) {
  ...
  console.log(`[consult-scout-fallback] reason=${reason} card_id=${cardContext.card?.id || '-'}`);
  return {
    stageOutput: cleaned,
    error: null,
...
function makeAnalystFallbackResult({ cardContext, stage1Facts, latencyMs, provider, fallback_from, reason }) {
  ...
  console.log(`[consult-analyst-fallback] reason=${reason} card_id=${cardContext.card?.id || '-'}`);
  return {
    stageOutput: cleaned,
    error: null,
    latencyMs: latencyMs || 0,
...
function makeRedTeamFallbackResult({ cardContext, stage1Facts, stage2, latencyMs, provider, fallback_from, reason }) {
  ...
  console.log(`[consult-redteam-fallback] reason=${reason} card_id=${cardContext.card?.id || '-'}`);
  return {
    stageOutput: cleaned,
    error: null,
    latencyMs: latencyMs || 0,
