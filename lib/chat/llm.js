...
const FORMAL_ENDING_MAPS = [
  ...
  // 단순 어미 (문장 끝 · 공백 전) — '적이다'가 '이다'보다 먼저 와야 '적'이 안 지워진다
  { re: /([가-힣])적이다\./g, to: '$1적이야.' },
  { re: /([가-힣])적이다(\s|$)/g, to: '$1적이야$2' },
  ...
  { re: /([가-힣])\s*이다\./g, to: (m, p1) => p1 + copulaBanmal(p1) + '.' },
  { re: /([가-힣])\s*이다(\s|$)/g, to: (m, p1, p2) => p1 + copulaBanmal(p1) + p2 },
];
