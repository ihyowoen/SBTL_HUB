...
// '도' 조사까지 소비한다 — "지역별로도 브리프"에서 토큰이 '지역별로'에서 멈추면, 앵커
// 없는 정규식이 남은 '도'를 건너뛰고 뒤의 맨 '브리프'에 매칭해 지역별·달을 통째로 잃고
// 일반 주간으로 폴백한다. 별로?도? = 별·별로·별도·별로도. R13: 주제별 추가.
const GROUP_TOKEN = "((?:지역|주제)\\s*별로?도?)?";
// 캡처된 구성 토큰 → group 값 ("지역별…"→region, "주제별…"→theme)
function parseGroupToken(tok) {
  if (!tok) return null;
  return String(tok).includes("주제") ? "theme" : "region";
}
...
function parsePeriodToken(tok) {
  ...
  if (tok === "월간" || tok === "30일") return { period: "monthly", month: null };
...
// 제안 칩용 최소 달 표기 — 클릭 시 이 라벨이 다시 parsePeriodToken으로 파싱되므로,
// bare "M월"이 같은 달로 되돌아올 때만 연도를 생략하고, 아니면 연도를 유지한다.
// (예: 2026-07-16에 "2025년 2월"호를 만들고 칩에서 연도를 떼면 "2월"→2026-02로 해석돼
//  방금 만든 달과 다른 호수를 만든다 — 과거 달일수록 이 왕복이 깨진다.)
function monthChipLabel(month) {
  const m = String(month || "").match(/^(\d{4})-(\d{2})$/);
  if (!m) return monthKo(month);
  const bare = `${Number(m[2])}월`;
  return parsePeriodToken(bare).month === month ? bare : `${m[1]}년 ${Number(m[2])}월`;
}
...
export function detectAppCommand(message, aliasGroups = [], opts = {}) {
  ...
  if (briefNowMatch) {
    ...
    const group = parseGroupToken(briefNowMatch[1] || briefNowMatch[3]);
  ...
  if (weeklyShowMatch) {
    ...
    const group = parseGroupToken(weeklyShowMatch[1] || weeklyShowMatch[3]);
...
export function appCommandResponse(cmd) {
  ...
  if (cmd.type === "brief_now") {
    ...
    const groupText = cmd.group === "region" ? " 지역별로 묶어서 정리할게." : cmd.group === "theme" ? " 주제별로 묶어서 정리할게." : "";
    ...
    return {
      ...
      suggestions: [
        ...
        // monthChipLabel로 연도를 보존한다(칩은 다시 파싱되므로 같은 달로 왕복해야 한다).
        cmd.month && cmd.group !== "region"
          ? { label: `${monthChipLabel(cmd.month)} 지역별 빠른 분석 만들어줘` }
          : { label: cmd.period === "monthly" ? "30일 빠른 분석 보여줘" : "주간 브리프 보여줘" },
  ...
  if (cmd.type === "brief_no_material") {
    ...
    return {
      answer: `${windowLabel} ${scopeText} 2장이 안 돼서 이번엔 분석 재료가 부족해. ${cmd.month ? "다른 달로 해보거나, " : ""}${cmd.scope === "all" ? "새 카드가 쌓이면" : "워치를 조금 넓히거나, 새 카드가 쌓이면"} 다시 말해줘.`,
      suggestions: [
  ...
  if (cmd.reason === "recent_brief") {
    return {
      answer: `방금 만든 ${showLabel || "빠른 분석"}이 있어 — 새로 만드는 대신 바로 보여줄게. 다시 뽑고 싶으면 10분쯤 뒤에 말해줘.`,
      suggestions: [
        { label: "내워치 관련 최근 소식 알려줘", hint_action: "new_query", hint_topic: "news" },
      ],
    };
  ...
  if (cmd.month) {
    // 칩·재요청 문구는 monthChipLabel로 연도를 보존 — 클릭 시 다시 파싱돼 같은 달로 왕복해야 한다
    const chipMonth = monthChipLabel(cmd.month);
    return {
      answer: `${monthKo(cmd.month)} 월별 빠른 분석 열어줄게 — 브리핑룸으로 이동할게. 그 달 분석이 아직 없으면 '${chipMonth} 빠른 분석 만들어줘'라고 해주면 바로 만들게.`,
      suggestions: [
        { label: `${chipMonth} 빠른 분석 만들어줘` },
        { label: "오늘 핵심 카드", hint_action: "new_query", hint_topic: "news" },
      ],
    };
  }
