...
// Model: llama-3.1-8b-instant (2026-04-20 교체 — 무료 tier TPM 3배)
//        품질 약간 하락하지만 한국어 반말·JSON mode 둘 다 OK.
//        70b로 되돌리려면 DEFAULT_MODEL 한 줄만 수정.
// Timeout: 9s (Vercel serverless 기본 10s 넘지 않게)
// Retry: 429 (rate-limit) 한 번만 0.8s 대기 후 retry.
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (status === 429 && _retryCount < 1) {
        ...
        const waitMs = 800;
        console.log(`[groq-retry] rate-limit, wait ${waitMs}ms then retry`);
        await sleep(waitMs);
        return callGroq({
      ...
      return { text: null, error: errType, status, latencyMs, retried: _retryCount > 0 };
    }

    const data = await res.json();
    const text = data?.choices?.[0]?.message?.content?.trim() || null;
    return { text, error: text ? null : "empty-response", latencyMs, retried: _retryCount > 0 };
  } catch (e) {
    const latencyMs = Date.now() - t0;
...
//
// ## v2.2 핵심 변경 (2026-04-20)
// - SBTL Chain 섹션 신설 → LLM이 "SBTL 관점 억지 엮기" 하지 않도록
// - 한자 leak 방지 예시 추가 ("密切", "激化" 등)
// - remaining_points SBTL 관점 의무화 제거 (카드 연결 명확할 때만)
// ============================================================================
