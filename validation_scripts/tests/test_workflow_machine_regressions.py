...
from card_audit_utils import (  # noqa: E402
    canonical_domain,
    canonical_url,
    is_landing_page,
    load_owner_registry,
...
from date_role_freshness_check import check_card as check_date_card  # noqa: E402
from related_lifecycle_check import check_card  # noqa: E402
from recompute_source_audit_metadata import recompute  # noqa: E402
from run_workflow_contract_suite import validate_scope  # noqa: E402
...
class SourceAuditRecomputeTest(unittest.TestCase):
    def test_recompute_preserves_rejected_discovery_rows(self):
        ...
        used = next(row for row in updated["source_discovery_ledger"] if row.get("outcome") == "used_in_fact_sources")
...
class DateRoleContractTest(unittest.TestCase):
    ...
    def test_follow_up_requires_anchor(self):
        ...
        self.assertTrue(any("fresh_follow_up_anchor" in error for error in check_date_card(card, True)))
...
class RelatedContractTest(unittest.TestCase):
    ...
    def test_legacy_mode_does_not_require_v2_follow_up_fields(self):
        ...
        strict_fields = ("fresh_follow_up_anchor_class", "incremental_fact_vs_predecessor", "changed_judgment_vs_predecessor")
        self.assertFalse(any(any(name in error for name in strict_fields) for error in errors))
...
class SuiteScopeTest(unittest.TestCase):
    ...
    def test_scope_requires_all_requested_ids_to_exist(self):
        with tempfile.TemporaryDirectory() as tmp:
            cards = self.write_json(tmp, "cards.json", {"cards": [{"id": "A"}, {"id": "B"}]})
