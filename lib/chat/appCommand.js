...
// R8: 피드 필터 명령 — "중국 카드만 보여줘"·"최근 7일로 좁혀줘"·"내워치 TOP만 보여줘".
// 지역/기간/시그널/내워치 4개 면(facet)만 다루고, 파싱 후 실질 잔여 텍스트(엔티티 등)가
// 남으면 null로 물러나 일반 검색이 처리하게 한다("CATL 카드 보여줘"는 검색으로).
const R8FEED_REGION = { "중국": "CN", "미국": "US", "북미": "NA", "한국": "KR", "유럽": "EU", "일본": "JP", "글로벌": "GL", "CN": "CN", "US": "US", "NA": "NA", "KR": "KR", "EU": "EU", "JP": "JP", "GL": "GL" };
...
function detectFeedFilter(msg) {
  ...
  const facets = {};
  let rest = msg;
  for (const [k, v] of Object.entries(R8FEED_REGION)) {
