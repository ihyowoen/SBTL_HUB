...
const COMMON_RULES = [
  ...
  "6. 뉴스 가치 판단: 단순 계약·착공·MOU라는 사실 자체에 비중을 두지 않는다. 우선 서술할 것은 시장구조·경쟁구도의 변화, 공급·수요·가격·가동률 신호, 기술·성능·안전·운영검증 결과, 미래 현금흐름·자산가치(장기 계약·금융종결·오프테이크), 법률·정책·시장접근 변화, 시스템 규모의 이정표, 흐름의 지속성, 의사결정 긴급성을 담은 카드다. 계약·착공은 가격 공개·첫 진입·구조 변화 같은 신호를 담을 때만 비중 있게 다루고, 그렇지 않으면 한 문장 안에서 묶어 처리한다.",
].join("\n");
