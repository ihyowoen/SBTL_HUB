...
// Stage 3 wrapper variants — 가독성 v2.
// 강차장 톤(반말 자연 어투) + 메타 용어 최소화.
// "stage2 결론은" / "stage1 facts" 같은 시스템 용어를 "2차 해석" / "1차 사실"로.
// premature/unverified는 fact id 노출 안 함 (자연 표현 "두 사실" / "그 묶음").
// counter_scenario만 fact id 강제 (validator 요구).
const PREMATURE_WRAPPERS = [
  '근데 잠깐 — 2차 해석이 좀 빨랐을 수도. 두 사실을 한 방향으로 묶었다는 그 전제가, 1차에서 직접 잡힌 건 아니거든',
  '2차에서 잡은 {angle} 각도, 한 발 빼서 보면 좀 더 따져볼 자리야. 사실들이 정말 같은 방향인지가 1차 안에서 직접 안 갈라져',
  '2차 결론이 깔끔해 보여도 살짝 의심할 자리가 있어. 사실들을 한 묶음으로 본다는 그 출발점부터 1차에서 확정된 건 아니야',
];

const UNVERIFIED_WRAPPERS = [
  '한 가지 깔린 가정이 있어. 2차는 사실들이 같은 신호라는 걸 전제로 깔고 갔거든. 1차 사실만으로는 그게 직접 안 잡혀',
  '2차가 표면에 안 꺼낸 전제 — {angle} 각도가 다른 각도보다 더 정확하다는 거. 점수 비교가 명시적으로는 안 드러나',
  '슬쩍 깔린 가정 하나 — 사실들을 한 시그널로 묶을 수 있다는 점. 1차에서 충분히 안 갈라졌는데 2차가 그걸 한 방향으로 합쳤어',
];

// counter_scenario는 fact id 참조 필수. {ref}를 자연스럽게 본문에 녹여야 함.
// altImp는 30자 압축 + 작은따옴표로 인용. '쪽으로' 받침 무관 자연 결합.
const COUNTER_WITH_ALT = [
  "다른 시각도 있어. 카드의 다른 함의 '{altImp}' 쪽으로 읽으면 {ref}는 한 묶음이 아닐 수도 있고, {angle} 각도는 흐려져",
  "같은 사실도 '{altImp}' 쪽으로 묶으면 그림이 달라져. {ref}가 정말 한 방향인지 별도 확인이 필요한 자리야",
];

const COUNTER_GENERIC = [
  '{ref}를 한 묶음이 아니라 별개 사건으로 떼어서 보면 {angle} 각도는 약해져. 결론을 좁히기 전에 두 fact가 정말 한 방향인지 확인 필요',
  '{ref}를 따로 떼어서 읽으면 {angle} 시각은 흐려져. 두 fact의 실제 연결을 별도로 검증해보는 게 안전한 자리야',
];
