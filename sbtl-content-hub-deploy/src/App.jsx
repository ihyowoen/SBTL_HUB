...
const WEBTOON_COLLECTIONS = [
  {
    ...
    title:"LFP의 불편한 진실",
    subtitle:"오해 2개부터 비용 구조까지, LFP를 너무 쉽게 보면 놓치는 핵심을 시리즈 단위로 정리",
    badge:"FLAGSHIP SERIES",
    color:"#F85149",
    landingUrl:"/webtoon/lfp-uncomfortable-truths.html",
    status:"NOW LIVE",
    hook:"좋은 배터리일 수 있다. 그런데 너무 쉽게 보면 핵심을 놓친다.",
    description:"첫 공개는 EP.1 → EP.2로 잡고, Bonus 1을 다음 클릭으로 이어붙이는 구조가 가장 강합니다.",
    items:[
...
function Home({onNav,kb,dark}) {
  ...
  const featuredItems=featured.items.slice(0,2);
  ...
  return (<div style={{padding:"0 14px 120px"}}>
    ...
    <div style={{background:`linear-gradient(135deg, ${dark?"#151B2B":"#ffffff"}, ${dark?"#1F2840":"#EEF3FF"})`,borderRadius:16,padding:"16px",border:`1px solid ${dark?"#2C3550":t.brd}`,marginBottom:12,boxShadow:t.sh}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:8}}>
            <span style={{fontSize:9,fontWeight:900,color:"#000",background:"#F85149",padding:"4px 8px",borderRadius:999,fontFamily:"'JetBrains Mono',monospace"}}>{featured.status}</span>
            <span style={{fontSize:10,color:t.sub,fontFamily:"'JetBrains Mono',monospace"}}>FEATURED SERIES</span>
          </div>
          <div style={{fontSize:20,fontWeight:900,color:t.tx,lineHeight:1.25}}>{featured.title}</div>
          <div style={{fontSize:12,color:t.sub,lineHeight:1.6,marginTop:6}}>{featured.hook}</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
            {featured.items.map(item=><span key={item.id} style={{fontSize:9,color:t.tx,background:dark?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.04)",padding:"5px 8px",borderRadius:999,fontFamily:"'JetBrains Mono',monospace"}}>{item.tag}</span>)}
          </div>
        </div>
        <div style={{width:54,height:54,borderRadius:16,background:dark?"rgba(248,81,73,0.12)":"rgba(248,81,73,0.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>📁</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
        {featuredItems.map(item=>(
          <div key={item.id} onClick={()=>window.open(item.url,"_blank")} style={{background:dark?"rgba(10,14,20,0.65)":"rgba(255,255,255,0.9)",border:`1px solid ${t.brd}`,borderRadius:12,padding:"11px",cursor:"pointer"}}>
            <div style={{fontSize:9,fontWeight:800,color:item.color,fontFamily:"'JetBrains Mono',monospace"}}>{item.tag}</div>
            <div style={{fontSize:13,fontWeight:800,color:t.tx,lineHeight:1.35,marginTop:5}}>{item.title}</div>
            <div style={{fontSize:10,color:t.sub,lineHeight:1.45,marginTop:5}}>{item.subtitle}</div>
          </div>
        ))}
      </div>
      <div style={{display:"flex",gap:8,marginTop:12}}>
        <button onClick={()=>onNav("webtoon")} style={{flex:1,border:"none",borderRadius:10,padding:"10px 12px",background:t.cyan,color:"#000",fontSize:11,fontWeight:900,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>OPEN FOLDER</button>
        <button onClick={()=>window.open(featured.items[0].url,"_blank")} style={{flex:1,border:`1px solid ${t.brd}`,borderRadius:10,padding:"10px 12px",background:"transparent",color:t.tx,fontSize:11,fontWeight:900,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>START EP.1 ↗</button>
      </div>
      <div style={{marginTop:8,fontSize:9,color:t.sub,fontFamily:"'JetBrains Mono',monospace"}}>NEXT CLICK → BONUS.01 누가 진짜 돈을 내나?</div>
    </div>
...
function CollectionFolder({collection,dark,defaultOpen=false}){const t=T(dark);const[open,setOpen]=useState(defaultOpen);return(<div style={{background:t.card2,borderRadius:16,border:`1px solid ${t.brd}`,overflow:"hidden",boxShadow:t.sh}}>
  ...
  <div style={{padding:"16px"}}>
    ...
    <div style={{display:"flex",gap:8,marginTop:14}}>
      <SeriesActionButton label="OPEN SERIES PAGE ↗" onClick={()=>window.open(collection.landingUrl,"_blank")} primary dark={dark}/>
      <SeriesActionButton label="START EP.1 ↗" onClick={()=>window.open(collection.items[0].url,"_blank")} dark={dark}/>
    </div>
  </div>
...
function LegacyShelf({items,dark}){const t=T(dark);const[open,setOpen]=useState(false);return(<div style={{background:t.card2,borderRadius:12,border:`1px solid ${t.brd}`,overflow:"hidden"}}>
  <button onClick={()=>setOpen(v=>!v)} style={{width:"100%",textAlign:"left",background:"transparent",border:"none",padding:"14px 16px",cursor:"pointer",color:t.tx,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
    <div>
      ...
      <div style={{fontSize:11,color:t.sub,marginTop:4}}>새 시리즈 폴더와 분리해서 보관. 필요할 때만 펼쳐 보면 돼.</div>
    </div>
...
function WebtoonHero({collection,dark}){const t=T(dark);return(<div style={{background:`linear-gradient(135deg, ${dark?"#151B2B":"#ffffff"}, ${dark?"#232D47":"#EEF3FF"})`,borderRadius:16,padding:"16px",border:`1px solid ${dark?"#2C3550":t.brd}`}}>
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:14}}>
    <div style={{flex:1,minWidth:0}}>
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:8}}>
        <SmallActionPill label="SERIES SHELF" dark={dark}/>
  ...
  <div style={{display:"flex",gap:8,marginTop:14}}>
    <SeriesActionButton label="SERIES PAGE ↗" onClick={()=>window.open(collection.landingUrl,"_blank")} primary dark={dark}/>
    <SeriesActionButton label="EP.1 바로 보기 ↗" onClick={()=>window.open(collection.items[0].url,"_blank")} dark={dark}/>
  </div>
</div>);}
  ...
  <div style={{background:t.card2,borderRadius:12,padding:"14px 16px",border:`1px solid ${t.brd}`}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:6}}>
      <div>
        ...
        <h2 style={{fontSize:18,fontWeight:900,color:t.tx,margin:0}}>시리즈 폴더</h2>
      </div>
      <div style={{fontSize:10,color:t.sub,fontFamily:"'JetBrains Mono',monospace"}}>{WEBTOON_COLLECTIONS.length} COLLECTION</div>
    </div>
    <p style={{fontSize:12,color:t.sub,lineHeight:1.6,margin:0}}>지금처럼 낱개 에피소드를 계속 늘리는 대신, 시리즈 단위로 묶어서 안에 완성본을 넣는 구조. 나중에 EP.3, Bonus 2, Bonus 3도 같은 폴더 안으로 자연스럽게 확장할 수 있어.</p>
  </div>
