...
//   GEMINI_API_KEY 설정 → Gemini 2.5 Flash 사용 (primary, 1M TPM · 10 RPM)
//   GEMINI_API_KEY 없고 GROQ_API_KEY 있으면 → Groq 사용 (legacy fallback)
//   둘 다 없으면 → null 반환 ("no-api-key")
//
// Vercel env 세팅:
//   Settings → Environment Variables → Add `GEMINI_API_KEY` (Production + Preview)
//   Google AI Studio (https://aistudio.google.com/apikey)에서 발급
//
// Timeout: 9s. Retry: 429 (rate-limit) 한 번만 0.8s 대기 후 retry.
// 실패 시 null 반환 → 호출자가 템플릿 fallback으로 graceful degrade.
// ============================================================================
//
// Persona Roles (2026-04-20 업데이트 — Coverage Contract + SBTL Chain)
// - SYSTEM_CHAT        : SBTL 사내 시니어 애널리스트 (30+ yrs)
// - SYSTEM_CARD_CONSULT: 배터리 상담소 상담사 '강차장' (JSON + Coverage + Chain)
...
/**
 * Gemini Chat Completion 호출.
 * 반환: { text, error, latencyMs, provider, retried }
 *
 * - responseFormat={type:"json_object"} 시 Gemini responseMimeType=application/json 사용.
 * - systemInstruction 으로 system prompt 분리 전달.
 * - 429 (rate-limit) 시 800ms 대기 후 1회 retry.
 */
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (status === 429 && _retryCount < 1) {
        ...
        console.log(`[gemini-retry] rate-limit, wait 800ms then retry`);
        await sleep(800);
        return callGemini({
      ...
      return { text: null, error: errType, status, latencyMs, provider: "gemini", retried: _retryCount > 0 };
    }

    const data = await res.json();
    // Gemini 응답 구조: candidates[0].content.parts[0].text
    // 안전장치: prompt/safety 블록 시 finishReason 체크
...
export async function callGroq({
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (status === 429 && _retryCount < 1) {
        clearTimeout(timeout);
        console.log(`[groq-retry] rate-limit, wait 800ms then retry`);
        await sleep(800);
        return callGroq({
...
// ============================================================================
// LLM dispatcher — provider auto-select
// ============================================================================
//
// Strategy:
//   GEMINI_API_KEY 설정됨 → callGemini
//   아니면 GROQ_API_KEY 설정 → callGroq
//   둘 다 없으면 → {text:null, error:"no-api-key"}
//
// 호출자는 provider 신경 안 써. opts shape 동일, return shape 동일.
// ============================================================================
...
- label은 **7~14자** 반말 짧은 질문/여지 형태. 보고서 제목 금지.
  ...
  나쁜 예 (15자+ 보고서 제목형): "CATL의 광물 자회사 설립이 파우치 업체에 미치는 영향", "충전기 설치 대수보다 가동률·수익성 평가"
...
/**
 * 단일 카드 상담 LLM 호출 — Coverage Contract v2.2.
 */
export async function synthesizeCardConsult({
  ...
  // 디버그: provider/error 1줄 로그
  if (result?.error) {
    console.log(`[llm] provider=${result.provider} error=${result.error} latency=${result.latencyMs}ms isOpener=${isOpener}`);
  }
