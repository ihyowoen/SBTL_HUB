...
def common_item(event, j, strict=False):
    ...
    baseline_follow=rel['relation_type'] if rel else 'new_unrelated_event'
    ...
    item={
      ...
      'stage_b_requirement_note':'Stage A is selector-only. No fact_sources, source quotes or card copy were created; strict items still require a Stage B evidence package.',
...
summary={
 ...
 'high_value_review_pool_ids':[i['review_pool_item_id'] for i in review_items if i['decision_news_value_score']>=55],
...
artifact={
 ...
 'source_universe':'R6 corrected 395-event universe; Formal Stage A Batch 01 covers ordinals 1-25 and all source-bound observations assigned to those events','story_count':len(ledger),'event_count':25,'original_status_counts':{'r6_batch_events':25,'source_bound_observations':len(ledger)},
...
full_errors=validate_full_stage_a_artifact(artifact,lineage._compat_module)
rc=lineage.check_stage_a(artifact)
report={'schema':'formal_stage_a_r6_batch01_validation_v1','status':'PASS' if not(pre_errors or v4_errors or hard_errors or full_errors) and rc==0 else 'FAIL','artifact':str(OUT.relative_to(ROOT)),'artifact_sha256':hashlib.sha256(OUT.read_bytes()).hexdigest(),'event_count':25,'source_bound_observation_count':len(ledger),'strict_count':7,'candidate_review_count':11,'watchlist_count':6,'reject_or_support_only_count':1,'prevalidation_errors':pre_errors,'v4_contract_errors':v4_errors,'v4_hardening_errors':hard_errors,'full_completeness_errors':full_errors,'lineage_check_rc':rc,'external_web_search_count':0,'article_body_fetch_count':0}
