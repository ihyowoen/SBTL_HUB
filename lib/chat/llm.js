...
// ============================================================================
//
// Persona Roles (2026-04-18 업데이트) — 용도별 차등
// - SYSTEM_CHAT        : SBTL 사내 시니어 애널리스트 (30+ yrs)
// - SYSTEM_CARD_CONSULT: SBTL 사내 시니어 애널리스트 (단일 카드 상담)
// - ANALYSIS why       : MBB급 글로벌 시니어 베테랑 파트너
// - ANALYSIS summary   : 사내 브리핑 담당자 (중립 유지)
// - ANALYSIS analysis  : GS/MS급 시니어 산업 애널리스트 + 프리미엄 뉴스레터 EIC
...
// ============================================================================
// Dedicated single-card consult synthesis
// ============================================================================

// ─── SYSTEM_CARD_CONSULT: SBTL 사내 시니어 애널리스트 ────────────────────
const SYSTEM_CARD_CONSULT = `너는 SBTL첨단소재의 사내 시니어 애널리스트 '강차장'이야.
K-배터리·ESS·소부장을 30년 넘게 봐온 베테랑이고, C-level·기관투자자 눈높이로 산업 흐름을 읽어.
사용자가 특정 카드 하나를 들고 와서 설명을 요청한 상황이야.

${TONE_RULE}

[답변 규칙]
- 반드시 주어진 카드 1장만 기준으로 답해. 일반론으로 새지 마.
- 답변 순서는 항상 다음 3개야.
  1) 무슨 일인지
  2) 왜 중요한지
  3) 다음 체크포인트 2개
- 전체는 5~8문장.
- 카드에 없는 내용은 억지로 만들지 말고, 부족하면 "카드 기준으로는 여기까지 보여"처럼 솔직히 말해.
- 번호, 불릿, 마크다운 없이 자연스러운 짧은 단락으로 써.`;

export async function synthesizeCardConsult({ card, userAsk = "" } = {}) {
  if (!card) return { text: null, error: "no-card", latencyMs: 0 };

  const parts = [
    `사용자 요청: ${userAsk || "이 카드 기준으로 설명해줘."}`,
    "",
    "[카드 정보]",
    `제목: ${card.T || card.title || ""}`,
    `부제: ${card.sub || ""}`,
    `의미: ${card.g || ""}`,
    `날짜: ${card.d || card.date || ""}`,
    `지역: ${card.r || card.region || ""}`,
    `출처: ${card.src || card.source || ""}`,
  ];
...
// ============================================================================
//
// why      : MBB급 글로벌 시니어 베테랑 파트너 — 전략적 의미·파급
// summary  : 사내 브리핑 담당자 — 팩트 중심, 중립 유지
// analysis : GS/MS급 시니어 산업 애널리스트 + 프리미엄 뉴스레터 EIC — 심층
// ============================================================================
...
// ============================================================================
// 사용처: Layer 4 synthesize.js의 action=rephrase 분기.
// 사용자 지시어("더 쉽게", "짧게", "예시로", "초등학생이 이해하게" 등)를 LLM에
// 전달해 직전 답변 본문만 재작성. 사실·숫자 유지, 어미·구성만 변경.
// ============================================================================
