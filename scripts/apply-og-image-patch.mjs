...
replaceOnce(
  "api/chat.js",
  String.raw`    const synthesis = await synthesize({
      parsed,
...
`,
  String.raw`    // OG fetch는 LLM 합성과 병렬 실행해 체감 지연을 최소화한다. trustedCards는
...
const oldCards = String.raw`                  {m.cards?.map((card, j) => {
...
replaceOnce("src/App.jsx", oldCards, newCards);
