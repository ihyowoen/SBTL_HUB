...
export async function loadKnowledge() {
  const [cardsRes, faqRes, trackerRes, regionPolicyRes] = await Promise.all([
    loadOne("cards.json"),
    loadOne("faq.json"),
  ...
  // 진단 로그 — source 분포 확인용
  console.log(
    `[loadKnowledge] cards=${cards.length}(${cardsRes.source}) faq=${faq.length}(${faqRes.source}) tracker=${tracker.items.length}(${trackerRes.source}) regionPolicy=${Object.keys(regionPolicy).length}keys(${regionPolicyRes.source})`
  );
