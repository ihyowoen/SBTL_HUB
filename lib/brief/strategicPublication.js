...
function validateSignal(errors, signal, path, { requireId = true } = {}) {
  if (!signal || typeof signal !== "object" || Array.isArray(signal)) {
    push(errors, path, "must be an object");
    return;
  }
  if (requireId && !nonEmptyString(signal.id)) push(errors, `${path}.id`, "is required");
...
function validateSignalEvidence(errors, signals, sourceIds, path) {
  (signals || []).forEach((signal, si) => {
    (signal?.card_ids || []).forEach((id, ci) => {
      if (!sourceIds.has(id)) push(errors, `${path}[${si}].card_ids[${ci}]`, "must exist in source_card_ids");
    });
  });
}
...
export function validateStrategicBriefItem(item, path = "item") {
  ...
  if (!nonEmptyString(item.conclusion)) push(errors, `${path}.conclusion`, "is required");
  ...
  if (!Array.isArray(item.structural_signals) || item.structural_signals.length < 1) {
    ...
    item.structural_signals.forEach((signal, i) => {
      validateSignal(errors, signal, `${path}.structural_signals[${i}]`);
      if (nonEmptyString(signal?.id)) {
        if (signalIds.has(signal.id)) push(errors, `${path}.structural_signals[${i}].id`, "must be unique within the issue");
        signalIds.add(signal.id);
      }
    });
  }

  if (item.regional_signals != null) {
    if (!Array.isArray(item.regional_signals)) push(errors, `${path}.regional_signals`, "must be an array when present");
    else item.regional_signals.forEach((signal, i) => validateSignal(errors, signal, `${path}.regional_signals[${i}]`, { requireId: false }));
  }
  ...
  // Cross-field evidence integrity. Official content has one governed source set:
  // every signal reference must point into it, and refs[] must be a one-to-one public
  // resolution table for the same card IDs. This blocks detached evidence rows and
  // structural claims that silently depend on cards outside the approved issue scope.
  if (Array.isArray(item.source_card_ids)) {
    const sourceIds = new Set(item.source_card_ids);
    validateSignalEvidence(errors, item.structural_signals, sourceIds, `${path}.structural_signals`);
    if (Array.isArray(item.regional_signals)) validateSignalEvidence(errors, item.regional_signals, sourceIds, `${path}.regional_signals`);
...
export function validateStrategicBriefLibrary(library) {
  ...
  if (library.generated_at != null && !validIsoDate(library.generated_at)) push(errors, "library.generated_at", "must be null or a real YYYY-MM-DD calendar date");
