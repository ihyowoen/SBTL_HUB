...
0.0D has one machine-verifiable responsibility: deterministically inventory/classify `docs/**`, lock every active authority to a git blob SHA, and prove the registry/baseline is internally consistent.

The lock also declares a small `bootstrap_read[]` context plan for the operator/model. Consuming that context is a runtime transition obligation before 0.0C, **not something the 0.0D machine artifact claims to prove**.
...
- exact bootstrap-context path set;
...
The helper does **not** prove that a model cognitively read any document. No machine field is to be interpreted that way.

## 3. Bootstrap context — runtime transition obligation, not PASS evidence

After the machine lock verifies and before executing 0.0C, load the exact `bootstrap_read_paths` emitted in the artifact. That list is registry-derived and intentionally much smaller than the full locked authority set.
...
`status=PASS` and `stage_0_0c_authorized=true` in the machine-generated 0.0D artifact mean the **repository-state preflight is eligible to proceed**. They do not attest that an LLM consumed the bootstrap context. The operator/model still must perform that load before executing 0.0C.
...
- `all_docs_files_read_or_parsed = true` means the deterministic helper completed the required git-tree inventory/classification/blob-lock parse. It does **not** claim that an LLM deep-read every active body or bootstrap file.
...
- `active_full_read_count` may be emitted only by the deterministic helper as a temporary compatibility mirror equal to `locked_authority_count`. It is **not** an authorization field and must never be interpreted as proof that the LLM read that many documents.
- `locked_authority_count` is the number of active authorities cryptographically bound to the run.
- `bootstrap_read_count` is the size of the declared bootstrap context plan. It is **not** a read counter.

The production gate requires a valid `governance_lock`; legacy count-only self-attestation is not accepted.
...
- exact registry bootstrap-context set;
- zero active override/addendum runtime dependencies;
- Stage A embedded news-value/Related contract verified;
- all machine blocker arrays empty.

After machine PASS, bootstrap-context loading remains a mandatory operator/model transition step before 0.0C, but it is deliberately **not represented as a machine-proven read attestation**.
