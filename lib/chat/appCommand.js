...
// 일반 주간으로 폴백한다. 별로?도? = 별·별로·별도·별로도.
const GROUP_TOKEN = "(지역\\s*별로?도?)?";
...
export function appCommandResponse(cmd) {
  ...
  if (cmd.month) {
    ...
    return {
      answer: `${monthKo(cmd.month)} 브리프 열어줄게 — NEWS 탭으로 이동할게. 그 달 호수가 아직 없으면 '${chipMonth} 브리프 만들어줘'라고 해주면 바로 만들게.`,
      suggestions: [
