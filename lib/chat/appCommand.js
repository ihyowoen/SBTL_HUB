...
// 열람도 기간을 캡처(그룹 1) — "월간 브리프 보여줘"가 주간호를 열면 안 된다.
// 기간을 안 밝히면(그냥 "브리프 보여줘") period 없이 반환해 최신호를 그대로 연다.
const CMD_WEEKLY = new RegExp(`(주간|월간)?\\s*브리프(?:을|를)?\\s*(?:보여|열어|볼|줘|확인)${VIEW_TAIL}`);
// 강제 발행("지금 브리프 만들어줘"·"월간 브리프 만들어줘") — 열람(CMD_WEEKLY: 보여/열어…)과
// 동사가 달라 충돌 없음. 생성은 LLM 호출을 유발하므로 워치 변이와 같은 명령형 앵커를 적용
// (질문형 "만들면 뭐가 좋아?"는 검색으로). '월간'이면 재료 창 30일, 아니면 주간(7일).
// 기간(주간|월간)은 캡처 그룹 1 — '브리프' 바로 앞에 붙은 토큰만 기간으로 인정한다.
// 메시지 전체를 /월간/으로 훑으면 "월간 말고 주간 브리프 만들어줘"가 월간으로 오판된다
// (R6의 "CATL 말고 BYD 워치"와 같은 계열 — 대상은 키워드에 인접한 토큰에서 읽어야 한다).
const CMD_BRIEF_NOW = new RegExp(`(주간|월간)?\\s*브리프(?:을|를)?\\s*(?:지금\\s*|새로\\s*|다시\\s*|하나\\s*)*(?:만들어|만들|생성|발행|뽑아)${IMPERATIVE_TAIL}`);
...
export function detectAppCommand(message, aliasGroups = [], opts = {}) {
  ...
  if (briefNowMatch) {
    ...
    // 워치 없이도 브리프를 경험하게). 범위·기간은 응답 문구와 클라 생성기에 그대로 전달.
    const wt = opts.watchTerms || [];
    const period = briefNowMatch[1] === "월간" ? "monthly" : "weekly"; // 매치된 '브리프' 앞 토큰만
    const days = period === "monthly" ? 30 : 7;
    const scope = wt.length ? "watch" : "all";
    // 쿨다운은 기간별 분리 — 주간 발행 직후의 월간 요청은 정당하다
    const lastAt = Number((period === "monthly" ? opts.lastMonthlyBriefAt : opts.lastBriefAt) || 0);
    // 쿨다운으로 열람 전환할 때도 요청한 기간을 유지 — 월간을 요청했는데 주간호를 열지 않도록
    if (lastAt && Date.now() - lastAt < BRIEF_RECENT_MS) return { type: "weekly_show", reason: "recent_brief", period };
    const material = scope === "watch" ? countRecentWatchMatches(opts.cards || [], wt, days) : countRecentCards(opts.cards || [], days);
    if (material < 2) return { type: "brief_no_material", scope, period };
    return { type: "brief_now", scope, period };
  }
  const weeklyShowMatch = msg.match(CMD_WEEKLY);
  if (weeklyShowMatch) {
    // 기간을 명시한 열람만 period를 실어 보낸다 — 없으면 클라가 최신호(기간 무관)를 연다
    const p = weeklyShowMatch[1];
    return p ? { type: "weekly_show", period: p === "월간" ? "monthly" : "weekly" } : { type: "weekly_show" };
  }
...
export function appCommandResponse(cmd) {
  ...
  if (cmd.type === "brief_now") {
    const periodLabel = cmd.period === "monthly" ? "월간" : "주간";
    const windowLabel = cmd.period === "monthly" ? "최근 30일" : "최근 7일";
    const scopeText = cmd.scope === "all" ? `${windowLabel} 전체 카드(워치가 비어 있어서 전체 기준이야)` : `${windowLabel} 내워치 카드`;
    return {
      answer: `응, 지금 ${periodLabel} 브리프 만들게. ${scopeText}를 모아 정리하는 데 수십 초 걸려 — NEWS 📮 선반 열어놓을 테니 완성되면 맨 위에 꽂아둘게.`,
      suggestions: [
        { label: "주간 브리프 보여줘" },
  ...
  if (cmd.type === "brief_no_material") {
    ...
    return {
      answer: `${windowLabel} 사이 ${scopeText} 2장이 안 돼서 이번엔 브리프감이 부족해. ${cmd.scope === "all" ? "새 카드가 쌓이면" : "워치를 조금 넓히거나, 새 카드가 쌓이면"} 다시 말해줘.`,
      suggestions: [
  ...
  const showLabel = cmd.period === "monthly" ? "월간" : cmd.period === "weekly" ? "주간" : "";
  ...
  // '주간'만 만든다. 월간은 오직 명시 발행 명령·워치룸 버튼으로만 생긴다. 월간에도 "다음
  // 접속 때 자동으로 만들어져"라고 하면 오지 않을 자동 발행을 약속하는 셈이라 거짓말이 된다.
