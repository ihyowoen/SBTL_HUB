...
expected_command = "  `python validation_scripts/stage_lineage_contract_check.py stage_a <STAGE_A_JSON>`."
command_pattern = re.compile(
    r"(?m)^  `*python validation_scripts/stage_lineage_contract_check\.py stage_a <STAGE_A_JSON>`\.$"
)
for prompt_path in (
    ...
    if len(matches) != 1:
        raise SystemExit(f"{prompt_path}: expected one Stage A lineage command, found {len(matches)}")
    p.write_text(command_pattern.sub(expected_command, text, count=1), encoding="utf-8")
...
"""
current = Path(validator_path).read_text(encoding="utf-8")
if old_confirmation in current:
    replace_once(validator_path, old_confirmation, new_confirmation)
elif new_confirmation not in current:
    raise SystemExit("confirmation validator block not found")
