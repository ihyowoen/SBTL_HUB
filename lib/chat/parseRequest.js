...
export function parseRequest({ message, context = {}, hint = {}, data = {} } = {}) {
  ...
  if (hintTopic) {
    ...
    } else if (topic === "policy" && hasFaqKeyword(lowerMsg, data?.faq) && rawMessage.length <= 20) {
      // "FEOC 언제 적용돼?" 같은 짧은 개념 질의가 policy regex에 걸려도 faq_concept 우선.
      // 단 news/comparison은 제외 — "LG엔솔 최근 소식", "LFP랑 NCM 비교"가 FAQ 정적 답변에
      // 새치기당해 실시간 카드 검색을 못 타던 결함 수정.
      topic = "faq_concept";
      reasons.push("topic_overridden_faq_keyword_short_policy_query");
    } else {
