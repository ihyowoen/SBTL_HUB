...
text = text.replace('    cid = str(card.get("id", ""))', '    cid = primary_card_identifier(card)', 1)
...
validator.write_text(text.replace(old_main, new_main, 1), encoding='utf-8')
...
'''
(ROOT / '.github/workflows/workflow-contract-validation.yml').write_text(canonical_workflow, encoding='utf-8')
for path in [ROOT / 'scripts/patch_review_4840119588.py', ROOT / '.github/workflows/patch-review-4840119588.yml']:
    if path.exists():
        path.unlink()
