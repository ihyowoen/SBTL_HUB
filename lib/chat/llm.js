...
// ============================================================================
// Provider strategy:
//   1. GEMINI_API_KEY 있으면 → Gemini 2.5 Flash 시도
//   2. Gemini가 5xx/rate-limit/timeout → Groq 자동 재시도 (GROQ_API_KEY 있으면)
//   3. GEMINI 없고 GROQ만 → 바로 Groq
//   4. 둘 다 없음 → null ("no-api-key")
//
// 모델 선택 이력:
//   - gemini-2.5-flash-lite (시도): 무료 tier 여유 좋지만 복잡한 system prompt 해석
//     실패. 섹션 라벨([opener], [few-shot opener] 등)을 응답에 echo 함. 지시 따름 약함.
//   - gemini-2.5-flash (현재): 무료 tier peak time 503 잦음. 지시 따름 우수.
//     503 시 자동 Groq fallback으로 UX 투명 처리.
// ============================================================================
...
- 이런 라벨은 시스템이 너한테 지시할 때 쓰는 구조일 뿐, 너의 응답 안에 들어가면 안 돼.
...
// ============================================================================
// LLM dispatcher — Gemini primary, auto-fallback to Groq on transient errors
// ============================================================================
...
// ============================================================================
// Prompt echo cleanup — LLM이 system prompt 섹션 라벨을 복사한 경우 제거
// ============================================================================
// Flash-Lite에서 발견된 문제이나 Flash에서도 간혹 발생 가능.
// 응답 시작 부분의 [라벨] 섹션을 벗기고, 여러 섹션이 연결된 경우 마지막 섹션만 사용.
// (마지막이 대개 실제 답변이므로)
// ============================================================================
...
function stripPromptEchoes(text) {
  ...
  // 만약 응답이 [후속 답변] 라벨 이후 실제 답변을 담고 있다면 그 뒷부분만 취함
  ...
  // JSON 블록이 본문에 섞여 있으면 제거 (opener JSON이 followup 응답에 leak된 경우)
...
// ============================================================================
// 배터리 상담소 — Card Consult synthesis (Coverage Contract v2.2)
// ============================================================================
...
- label은 **7~14자** 반말 짧은 질문/여지 형태. 보고서 제목 금지.
  좋은 예: "우리 파우치엔 영향?" (10), "양산 시점 어때" (8), "K-3사 노출 크게" (11), "예산 얼마 잡혔어" (9), "타이밍 묘한데" (7)
  나쁜 예: "이 이슈 요약" / "더 쉽게 설명" / "정리해줘" / "결론은?"
  나쁜 예 (15자+ 보고서 제목형): "CATL의 광물 자회사 설립이 파우치 업체에 미치는 영향"
...
- 정보가 부족하면 짧게 인정하고 끊어. "관찰해봐야 해" 같은 말을 여러 번 반복하지 마.
- 3단 구조(무슨 일 → 왜 중요 → 체크포인트) 강제하지 마. 유저 질문에 직접 답해.
- 메타텍스트 금지 ("네, 알겠습니다", "답변드리겠습니다" 등).
- 응답 맨 앞에 '[opener]', '[few-shot opener]', '[remaining_points]', '[후속 답변]' 같은 대괄호 섹션 라벨 절대 금지.
- 응답에 JSON 객체({...}) 절대 포함 금지 (followup은 순수 자연문만).`;
...
**매우 중요**: 응답은 순수 자연 문장만. 절대 다음 것들 포함하지 마:
- '[opener]', '[few-shot opener]', '[remaining_points]', '[후속 답변]' 같은 대괄호 섹션 라벨
- JSON 객체 ({ "opener": ... } 같은 것)
- opener JSON + followup 답변을 한 번에 쓰지 마. 오직 followup 답변만 써.`;
...
export async function synthesizeCardConsult({
  ...
  if (!isOpener) {
    // Followup: 섹션 라벨 echo cleanup
    if (result?.text) {
      const cleaned = stripPromptEchoes(result.text);
  ...
  try {
    ...
    if (validateOpenerJson(parsed)) {
      return {
        text: stripPromptEchoes(String(parsed.opener).trim()),
    ...
    return {
      text: stripPromptEchoes(result.text),
