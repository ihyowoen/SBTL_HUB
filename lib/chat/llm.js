...
// ============================================================================
//
// Persona Roles (2026-04-20 업데이트 — Coverage Contract 도입)
// - SYSTEM_CHAT        : SBTL 사내 시니어 애널리스트 (30+ yrs)
// - SYSTEM_CARD_CONSULT: 배터리 상담소 상담사 '강차장' (JSON output + Coverage Contract)
// - ANALYSIS *         : [dead code, UI trigger 없음]
// - FAQ_REWRITE        : 톤 변환 에디터
// - REPHRASE           : 브리핑 담당자
 ...
 * responseFormat이 {type:"json_object"}면 Groq JSON mode 사용 → LLM이 JSON만 반환.
 */
...
// ============================================================================
// 배터리 상담소 — Card Consult synthesis (2026-04-20 Coverage Contract)
// ============================================================================
//
// ## 북극성
// "한 번 물어봤는데 제대로 답하더라 → 다른 카드도 해볼까"
// 2~3턴 대화로 카드의 coverage 완성하고, 유저가 "해결됨" 느껴야 함.
//
// ## Coverage Contract (v2.1)
// - LLM이 opener에서 각도 1개를 앞세우되, 나머지 함의 포인트는 remaining_points로 선언.
// - remaining_points는 다음 턴에서 유저가 탭하면 그 각도로 깊이 파고들 hook label.
// - implication[] 비어있으면 fact·gate 기반으로 coverage 판단.
//
// ## 2-mode
//   - isOpener=true  : JSON output {opener, remaining_points[]}. temp 0.35.
//   - isOpener=false : plain text. temp 0.4. 관련 카드 자연 인용.
//
// ## 안전망
//   - 데이터 영역 fence (injection 방어)
//   - post-check: JSON parse 실패 / injection marker leak / opener 필드 없음 → null 반환
//   - null 반환 시 api/chat.js가 hardcoded fallback 제공
// ============================================================================
...
- remaining_points는 유저가 "다음에 탭해서 깊게 팔" 후속 각도다. 카드 내용에 직접 뿌리내린 것들.
...
- 2~3개. 각각 { "label": "...", "topic": "..." }
- label은 7~15자 반말. "이 카드는…" / "요약해줘" / "결론은" 금지.
  좋은 예: "우리 파우치엔 영향?" / "CATL은 어떻게 받아" / "양산 시점 따져보면" / "예산 얼마 잡혔어" / "K-3사 중 누가 노출 크게"
  나쁜 예: "이 이슈 요약" / "더 쉽게 설명" / "정리해줘" / "결론은?"
- SBTL 관점(파우치·K-3사) 최소 1개 권장, 억지 금지.
...
다음 JSON 형식으로만 응답. 그 외 텍스트·설명·마크다운·코드 블록 없이 순수 JSON.
{
  ...
  "remaining_points": [
    {"label": "7~15자 반말", "topic": "snake_case_id"},
    {"label": "...", "topic": "..."}
  ]
}
...
// 응답 후처리 — 전형적 injection 마커가 응답에 새어나왔으면 null 반환
...
function validateOpenerJson(parsed) {
  ...
  // remaining_points 각각 label+topic 필수
...
/**
 * 단일 카드 상담 LLM 호출 — Coverage Contract v2.1.
 *
 * @param {object} opts
 * @param {object} opts.cardContext  buildCardConsultContext output (필수)
 * @param {boolean} opts.isOpener    true면 첫 발화 (JSON), false면 후속 답변 (text)
 * @param {string} opts.userMessage  후속 답변 시 유저 질문 텍스트
 * @returns {Promise<{text, remainingPoints?, structured?, error, latencyMs}>}
 *   opener JSON 파싱 성공 시: { text: opener, remainingPoints: [...], structured: true }
 *   opener JSON 파싱 실패 시: { text: null, error: 'json-parse-failed', ... }  → 호출자가 fallback
 *   followup: { text, error, latencyMs } (기존 shape)
 */
export async function synthesizeCardConsult({
  ...
  // Injection leak post-check (opener raw text 또는 followup 둘 다)
  ...
  // opener — JSON parse 시도
  if (!result?.text) return result;  // 이미 error 상태
  ...
  try {
    ...
    // JSON은 파싱되지만 schema 위반 — opener 필드만이라도 살려서 사용
    ...
    // JSON parse 실패 — raw text가 JSON-ish면 유저에게 노출하지 말고 null
    const looksLikeJson = /^\s*[{[]/.test(result.text);
    if (looksLikeJson) {
      return { text: null, error: 'json-parse-failed', latencyMs: result.latencyMs };
    }
    // JSON mode인데 plain text로 왔음 → 일단 text로 사용 (degrade)
    return {
      text: result.text,
