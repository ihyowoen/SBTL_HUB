...
function buildScoutFallback({ card }) {
  ...
  if (Array.isArray(card.implication)) {
    for (const imp of card.implication) {
      ...
      facts.push({
        ...
        time: card.date || null,
        place: null,
        raw: clampLen(String(imp).trim(), 15, 110),
  ...
  // summary: card.gate 우선, 없으면 sub, 그 다음 title 조합
  let summary = String(card.gate || card.sub || '').trim();
  if (!summary) summary = `${subject} 관련 카드가 접수됐어. LLM 자동 요약은 안 됐고, 카드 작성자의 정리만 전달돼.`;
  summary = clampLen(summary, 30, 240, '카드 작성자의 정리만 전달돼.');

  return {
    summary,
    facts,
    unknowns: ['LLM 자동 추출이 안 돼서 카드 작성자가 정리한 시각만 전달됐어'],
  };
}
...
function buildAnalystFallback({ stage1Facts, card }) {
  ...
  const timeCount = facts.filter(f => f.time).length;
  const valueCount = facts.filter(f => f.value).length;
  let angle = '대비';
  if (timeCount >= 2) angle = '타이밍';
  else if (valueCount >= 1) angle = '숫자';
  else if (facts.some(f => /지분|계약|거래|공급/.test(String(f.action || '')))) angle = '구조';

  // fact id 참조 (최소 2개, 1개뿐이면 1개)
  const ids = facts.map(f => f.id);
  const refStr = ids.length >= 2 ? `${ids[0]}과 ${ids[1]}` : (ids[0] || 'f1');

  // interpretation 본문 — 카드 작성자 시각 그대로
  const cardImpFirst = (Array.isArray(card.implication) && card.implication[0]) || card.gate || '';
  const cardImpFirstTrim = clampLen(String(cardImpFirst), 0, 150);

  let interpretation = '';
  if (ids.length >= 2) {
    interpretation = `걸리는 건 카드 작성자가 ${angle} 각도로 정리한 함의야. ${refStr}을 묶어보면, ${cardImpFirstTrim} 이게 핵심으로 나오거든. LLM 자동 해석은 안 됐고, 카드 작성자의 시각만 그대로 전달된 상태야.`;
  } else {
    interpretation = `걸리는 건 카드 작성자가 정리한 함의야. ${refStr} 하나로만 보면, ${cardImpFirstTrim} 이게 핵심으로 나오거든. facts 빈약함 — LLM 자동 해석은 안 됐고, 카드 작성자의 시각만 그대로 전달된 상태야.`;
  }
  interpretation = clampLen(interpretation, 60, 380);

  // key_tension: card.gate 또는 함의 첫 줄 압축
  let keyTension = String(card.gate || cardImpFirstTrim || '').split(/[.。]/)[0].trim();
  keyTension = clampLen(keyTension, 8, 70, '카드 작성자 시각');

  return { angle, interpretation, key_tension: keyTension };
}

function buildRedTeamFallback({ stage1Facts }) {
  const facts = Array.isArray(stage1Facts) ? stage1Facts : [];
  const ids = facts.map(f => f.id);
  const refStr = ids.length >= 2 ? `${ids[0]}과 ${ids[1]}` : (ids[0] || 'f1');
  ...
  return {
    premature_interpretations: [
      'stage2 해석 자체가 LLM 없이 카드 작성자 시각만 전달했어 — 과속이라기보단 자기 검증 자체가 제한된 상태야',
    ],
    unverified_premises: [
      "'카드 작성자가 정리한 함의가 곧 가장 강한 각도다'는 전제 — stage1 facts 안에서 직접 확인되지 않음",
    ],
    counter_scenario: `${refStr}을 다른 각도로 다시 묶으려면 LLM 재시도가 필요해. 지금은 카드 작성자가 잡아둔 한 가지 시각만 전달된 상태고, 다른 읽기를 검증할 도구가 없는 상태야.`,
    next_checkpoints: [
      '배터리상담소 LLM 다시 호출해서 다른 각도 받아보기',
      '카드 원문 출처 직접 열어 누락된 사실 점검',
    ],
  };
}
...
function makeRedTeamFallbackResult({ cardContext, stage1Facts, latencyMs, provider, fallback_from, reason }) {
  const stageOutput = buildRedTeamFallback({ stage1Facts });
  const cleaned = {
