...
- 반드시 stage2 interpretation 또는 key_tension에서 짧은 인용을 작은따옴표로 감싸 포함
- 인용은 stage2 원문에서 그대로 가져와야 한다 (3자 이상 substring)
- 과속 지점은 "무엇을 건너뛰었나" 또는 "무엇을 전제로 깔았나" 관점

좋은 예 (구체):
"stage2 숫자 각도 해석 중 'f1과 f3을 묶어 업계 기준 대비 크다' 부분 — 수율·가동률 정보 없이 설비 용량만으로 규모 효과 단정"
"2차가 '2GWh 수주 의미'로 결론 좁혔는데 — f1·f3이 같은 시점이 아니라 다른 단계 데이터일 가능성 점검 안 됨"

나쁜 예 (Generic skeleton 금지):
"두 사실을 한 방향으로 묶었다는 그 전제가, 1차에서 직접 잡힌 건 아니거든" (어느 카드에든 똑같이 적용 가능. stage2 인용 없음)
"사실들이 정말 같은 방향인지가 1차 안에서 직접 안 갈라져" (stage2 specific 인용 없음)
"해석이 좀 급했다" (구체 지점 없음)
"더 신중해야 한다" (추상적 비판)
...
- 전제를 명시적으로 문장화 (작은따옴표로 감싸 명시)
- 전제 안에 stage2 key_tension 또는 interpretation의 키워드 포함 (3자 이상, 카드별로 달라야 함)
- facts 배열에 없는 연결고리를 짚기
- 막연한 "정보 부족"만 쓰지 마

좋은 예 (구체):
"'2GWh 설비가 실제 생산 가능 용량과 같다'는 전제 — stage1 facts 안에 수율·가동률 항목 없음"
"'표준 호환성이 보급 속도를 좌우한다'는 전제 — 2차가 깔았지만 1차 facts 안에 사용자 채택률·시장 비중 데이터 없음"

나쁜 예 (Generic skeleton 금지):
"사실들이 같은 신호라는 걸 전제로 깔고 갔거든" (어느 카드에든 적용 가능)
"한 시그널로 묶을 수 있다는 점" (추상적, stage2 인용 없음)
"정보가 더 필요하다" (전제 명시 없음)
...
- 가능하면 카드 implication 중 stage2가 사용한 것 외의 다른 항목을 alternative view로 인용한다.
  (카드에 implication 2개 이상이면 다른 implication 직접 인용)
- counter_scenario 자체에도 stage2 key_tension 또는 카드의 다른 함의에서 짧은 키워드(3자 이상) 인용 권장.
...
function validateRedTeamJson(parsed, stage1Facts = [], stage2 = null) {
  ...
  // ─── L3 specificity 검사 — stage2 key_tension/interpretation 키워드 인용 강제 ───
  // generic skeleton ("두 사실을 한 방향으로", "같은 신호로 묶었다") 차단.
  // pi 또는 up 중 적어도 하나가 stage2 specific 키워드 (3자 이상) 포함해야 함.
  if (stage2 && stage2.key_tension && typeof stage2.key_tension === 'string') {
    ...
    // stage2에서 추출한 키워드 후보 — 3자 이상 한글/영숫자 조각
    const ktKeywords = (kt.match(/[가-힣A-Za-z0-9]{3,}/g) || []).filter(w =>
      !['stage2', '2차', '각도', '해석', '결론', '핵심', '쟁점'].includes(w)
    );
    // interpretation 안의 5자 이상 substring도 후보로 (LLM이 직접 인용했을 때 잡기)
    const hasInterpSubstring = (s) => {
      if (!interp || interp.length < 5) return false;
      // interpretation에서 무작위 위치의 5-15자 슬라이스가 s에 포함되는지 검사
      // 단순화: interpretation 전체를 5자 단위로 슬라이딩
      for (let i = 0; i + 5 <= interp.length; i++) {
        const slice = interp.slice(i, i + Math.min(15, interp.length - i));
        if (slice.length >= 5 && s.includes(slice.slice(0, 5))) {
          // 5자 매칭은 우연일 수 있으니 7자로 강화
          for (let len = 7; len <= Math.min(15, slice.length); len++) {
            if (s.includes(interp.slice(i, i + len))) return true;
          }
        }
      }
      return false;
    };
    const hasSpecificQuote = (s) =>
      ktKeywords.some(kw => s.includes(kw)) || hasInterpSubstring(s);

    const piHasSpecific = pi.some(hasSpecificQuote);
    const upHasSpecific = up.some(hasSpecificQuote);
    if (!piHasSpecific && !upHasSpecific) {
      return { ok: false, reason: 'no-stage2-specific-quote' };
    }
  }
...
// Stage 3 wrapper variants — 가독성 v3 (Step 4-RT 품질 보증).
// 강차장 톤(반말 자연 어투) + stage2 key_tension 직접 인용으로 카드 specific화.
// LLM이 활성화돼도 fallback이 발동될 때를 위한 보험 layer.
//
// {kt} = stage2.key_tension (작은따옴표로 감싸 인용)
// {ref} = fact id 묶음 (f1·f2)
// {angle} = 2차가 잡은 각도
// {altImp} = stage2가 안 쓴 카드 함의 (counter용)
const PREMATURE_WRAPPERS = [
  '근데 잠깐 — 2차가 \'{kt}\' 쪽으로 결론 잡았는데, 이게 facts 안에서 직접 갈라진 게 아니라 묶어 본 결과거든. 그 묶음 자체를 의심해볼 자리야',
  '2차의 \'{kt}\' 결론, 한 발 빼서 보면 좀 더 따져볼 자리야. {angle} 각도가 정말 1차에서 직접 잡혔는지가 빈자리거든',
  '2차 결론 \'{kt}\'가 깔끔해 보여도, 거기 가는 사실 묶음 자체가 1차에서 확정된 건 아니야. 출발점부터 의심할 자리',
];

const UNVERIFIED_WRAPPERS = [
  '한 가지 깔린 가정 — \'{kt}\' 결론을 위해 2차는 facts가 같은 신호라는 걸 전제로 깔았어. 1차 사실만으로는 그게 직접 안 잡혀',
  '2차가 표면에 안 꺼낸 전제 — \'{kt}\'가 다른 각도보다 정확하다는 거. 점수 비교가 명시 안 돼서 슬쩍 깔린 가정이거든',
  '\'{kt}\' 쪽으로 가려면 사실들이 한 방향이어야 하는데, 그 가정이 1차에서 충분히 안 갈라졌어',
];

// counter_scenario는 fact id 참조 필수. {ref}를 자연스럽게 본문에 녹여야 함.
// altImp는 30자 압축 + 작은따옴표로 인용. '쪽으로' 받침 무관 자연 결합.
const COUNTER_WITH_ALT = [
  "다른 시각도 있어. 카드의 다른 함의 '{altImp}' 쪽으로 읽으면 {ref}는 한 묶음이 아닐 수도 있고, '{kt}' 결론은 흐려져",
  "같은 사실도 '{altImp}' 쪽으로 묶으면 그림이 달라져. {ref}가 정말 \"{kt}\" 한 방향인지 별도 확인이 필요한 자리야",
];

const COUNTER_GENERIC = [
  '{ref}를 한 묶음이 아니라 별개 사건으로 떼어서 보면 \'{kt}\' 결론은 약해져. 두 fact가 정말 한 방향인지 확인 필요',
  '\'{kt}\' 쥐고 있다면, {ref}를 따로 떼어 읽었을 때 그 그림이 유지되는지가 검증 포인트야',
];
...
function buildRedTeamFallback({ stage1Facts, stage2, card }) {
  ...
  // kt — stage2 key_tension을 짧게 압축. wrapper의 작은따옴표 안에 들어감.
  // 25자 + 단어 경계 + ellipsis (compactImp와 동일 정책).
  // 없으면 card.gate 첫 절. 둘 다 없으면 '{subject} {angle} 각도'.
  const ktRaw = String(stage2?.key_tension || card?.gate || '').trim();
  let kt = ktRaw.split(/[.。]/)[0].trim();
  // 종결어미 제거 (다.,어.,해.,이다.,한다.,된다., 있어, 됐어 등)
  ...
  if (!kt) kt = `${pickSubject(card)} ${angle} 각도`;
  ...
  //
  // P2 fix (Codex review): stage2._picked_implication_index가 직렬화 통해 사라질 수
  // 있어 (LLM path는 이 필드 모름; client serialization도 underscore prefix 흘릴 수
  // 있음). undefined일 때 angle 기반으로 동일 로직 재실행 — 18.3% 카드에서
  // analyst와 redteam이 같은 implication 인용하는 버그 fix.
  const imps = (Array.isArray(card?.implication) ? card.implication : []).filter(Boolean);
  ...
  let altImp = null;
  // 종결어미 제거 + 30자로 압축, 단어 경계 보존 + ellipsis
  // counter_scenario에 자연스럽게 녹이기 위해 짧게.
  const compactImp = (s) => {
    let t = String(s || '').trim();
...
function makeAnalystFallbackResult({ cardContext, stage1Facts, latencyMs, provider, fallback_from, finishReason, reason }) {
  ...
  const cleaned = {
    ...
    // P2 fix: _picked_implication_index도 흘려줘서 stage3가 다른 implication 인용
    // 가능하게. (직렬화로 사라져도 stage3가 angle로 재추론 가능 — buildRedTeamFallback)
