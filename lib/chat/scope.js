...
export function detectRegion(txt = "") {
  ...
  if (/(한국|국내|\bkr\b)/.test(l)) return "KR";
  if (/(미국|북미|\bus\b|america)/.test(l)) return "US";
  if (/(중국|\bcn\b|china)/.test(l)) return "CN";
  if (/(유럽|\beu\b|europe)/.test(l)) return "EU";
  if (/(일본|\bjp\b|japan)/.test(l)) return "JP";
  return null;
}
...
export function extractScope(message = "", context = {}) {
  ...
  const l = txt.toLowerCase();
  const region = detectRegion(txt) || ((/(방금|이전|직전|아까)/.test(l) && context?.region) ? context.region : null);
