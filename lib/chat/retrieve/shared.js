...
// 엔티티 후보 매칭 — ASCII 시작/끝 후보는 단어 경계를 수동 확인.
// "Xi" 같은 짧은 별칭이 "Flexibility" 내부에 매칭돼 엔티티 보너스를 받는 것 방지.
// 한글·한자 등 비ASCII 후보는 기존처럼 substring 허용.
const ENTITY_WORDCHAR = /[A-Za-z0-9]/;

export function entityTextMatch(hay, needle) {
  if (!hay || !needle) return false;
  const asciiStart = ENTITY_WORDCHAR.test(needle[0]);
  const asciiEnd = ENTITY_WORDCHAR.test(needle[needle.length - 1]);
  let idx = hay.indexOf(needle);
  if (!asciiStart && !asciiEnd) return idx !== -1;
  while (idx !== -1) {
    const before = hay[idx - 1];
    const after = hay[idx + needle.length];
    const leftOk = !(asciiStart && before && ENTITY_WORDCHAR.test(before));
    const rightOk = !(asciiEnd && after && ENTITY_WORDCHAR.test(after));
...
export function scoreCard(card, ws = [], scope = {}, now = Date.now()) {
  ...
  // 워치/별칭 확장처럼 '이 엔티티에 대해 묻는' 질의는 entity_weight를 높여
  // 일반어 토큰(관련/흐름 등) 우연 매칭이 엔티티 카드를 밀어내지 못하게 한다.
  const entityWeight = Number(scope?.entity_weight) > 0 ? Number(scope.entity_weight) : ENTITY_MATCH_BONUS;
  for (const eRaw of entityCandidates) {
    const e = String(eRaw || "").toLowerCase();
    if (!e || e.length < 2) continue;
    if (entityTextMatch(tl, e) || entityTextMatch(sl, e) || entityTextMatch(gl, e) || ks.some((k) => k.includes(e) || e.includes(k))) {
      score += entityWeight;
      break;
    }
  }
