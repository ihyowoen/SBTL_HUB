...
export function validateMonthlyBriefProvenance(library, resolver) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    (Array.isArray(item?.refs) ? item.refs : []).forEach((ref, j) => {
      ...
      const allowedTitleUrls = cardTitleSupportingUrls(card);
