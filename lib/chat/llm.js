...
export const TONE_RULE = `[말투 규칙 — 반드시 지킬 것]
...
async function callGemini({
  ...
  model = DEFAULT_GEMINI_MODEL, _retryCount = 0, deadlineAt = null,
} = {}) {
  ...
  // deadlineAt(호출자 함수의 시간 예산): 개별 fetch 타임아웃을 남은 예산으로 자른다 —
  // 내부 429/503 재시도가 timeoutMs를 '한 번 더' 쓰면 예산 초과로 플랫폼이 함수를
  // 죽여, 의도된 에러 응답 대신 정체불명 타임아웃이 나간다(R13 P1).
  const effTimeoutMs = deadlineAt ? Math.max(3000, Math.min(timeoutMs, deadlineAt - Date.now() - 500)) : timeoutMs;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), effTimeoutMs);
  const t0 = Date.now();
  ...
  try {
    const body = {
      ...
      generationConfig: {
        ...
        // Gemini 2.5 Flash는 thinking이 default ON이고 thinking tokens가
        // maxOutputTokens 안에 함께 카운트된다. 우리 prompt들은 짧은 JSON
        // output이 목적이므로 thinking을 끄지 않으면 thinking이 budget을
        // 거의 다 먹고 actual output이 mid-string에서 잘려 'Unterminated string'
        // JSON parse error로 떨어진다. 0으로 명시 disable.
    ...
    // 키는 쿼리스트링이 아니라 헤더로 — URL은 오류 메시지·로그에 통째로 남는 경로다
    ...
    if (!res.ok) {
      ...
      // 예산이 얼마 안 남았으면 내부 재시도 생략 — 에러를 정직하게 반환해 호출자가
      // 남은 시간 안에 의도된 응답(degraded/502)을 만들게 한다
      if ((status === 429 || status === 503) && _retryCount < 1 && !(deadlineAt && deadlineAt - Date.now() < 6000)) {
        clearTimeout(timeout);
        await sleep(1200);
        return callGemini({
          system, user, maxTokens, temperature, timeoutMs, responseFormat, model,
          _retryCount: _retryCount + 1, deadlineAt,
        });
      }
      return {
    ...
    return { text, error: null, latencyMs, provider: "gemini", finishReason, retried: _retryCount > 0 };
  } catch (e) {
    const latencyMs = Date.now() - t0;
...
export async function callGroq({
  ...
  responseFormat = null, _retryCount = 0, deadlineAt = null,
} = {}) {
  ...
  // 개별 타임아웃을 남은 함수 예산으로 자른다(callGemini와 동일 — R13 P1)
  const effTimeoutMs = deadlineAt ? Math.max(3000, Math.min(timeoutMs, deadlineAt - Date.now() - 500)) : timeoutMs;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), effTimeoutMs);
  const t0 = Date.now();
  ...
  try {
    ...
    if (!res.ok) {
      const status = res.status;
      // gemini 경로와 동등한 분류·본문 스니펫·로그 — groq만 뭉뚱그린 provider-error라
      // 폴백 실패의 실원인(400 요청 거부? 404 모델 폐기? 413 단일요청 TPM 초과?)을
      // 프로덕션에서 구별할 수 없었다(실측: 축 페이로드 폴백이 매번 502 provider-error,
      // 원인 미상). 413은 groq가 '요청 하나가 분당 토큰 한도 초과'일 때 쓰는 코드로,
      // 카드 40장 축 페이로드(≈1만 토큰)가 무료 티어 한도를 넘는 경우가 유력 후보다.
      let errType = "provider-error";
      if (status === 400) errType = "bad-request";
      else if (status === 401 || status === 403) errType = "auth-error";
      else if (status === 404) errType = "model-not-found";
      else if (status === 413) errType = "payload-too-large";
      else if (status === 429) errType = "rate-limit";
      else if (status >= 500) errType = "server-error";

      let errSnippet = null;
      try { errSnippet = (await res.text()).slice(0, 500); } catch (e) { /* ignore */ }
      console.log(`[groq-err] model=${model} status=${status} retry=${_retryCount} body=${errSnippet ? errSnippet.slice(0, 200) : "-"}`);

      if (status === 429 && _retryCount < 1 && !(deadlineAt && deadlineAt - Date.now() < 5000)) {
        clearTimeout(timeout);
        await sleep(800);
        return callGroq({
      ...
      return { text: null, error: errType, status, latencyMs, provider: "groq", retried: _retryCount > 0, detail: errSnippet };
    }
...
export async function callLLM(opts) {
  ...
  // allowFallback=false면 groq 폴백을 건너뛴다 — 대용량 요청(축 브리프 ~11K+ 토큰)은
  // groq 무료 티어 per-request TPM(6~12K)을 넘어 413(payload-too-large)로 확정 실패한다.
  // 그런 요청은 groq를 시도해봐야 낭비이므로, 호출자가 미리 폴백을 끈다(gemini 전용).
  // groq만 있는 환경에서 대용량이면 애초에 못 하므로 그대로 진행(에러가 정직하게 나감).
  const allowFallback = opts?.allowFallback !== false;

  if (!hasGemini) return callGroq(opts);

  const geminiResult = await callGemini(opts);
  if (geminiResult?.text || !hasGroq || !allowFallback) return geminiResult;
  if (!FALLBACKABLE_ERRORS.has(geminiResult?.error)) return geminiResult;
  // 남은 예산이 폴백 한 번을 돌릴 수 없으면 gemini 에러를 그대로 — 플랫폼 타임아웃 방지
  if (opts?.deadlineAt && opts.deadlineAt - Date.now() < 4000) return geminiResult;
...
// 종결형만 치환한다 — 경계 없이 바꾸면 활용형이 깨진다(R28 실증: "중요하다고 평가"→
// "중요해고 평가", "확대된다면"→"커져면"). 활용형("~하다고/하다는/한다니")은 문어체로
// 남는 쪽이 문법 파괴보다 낫다. 경계 = 공백·구두점·따옴표·괄호·문자열 끝.
// 경계 = '다음 문자가 한글 음절이 아니면'(부정 lookahead). 활용을 이어가는 문자는 한글
// 음절(가-힣)뿐이므로 — "중요하다고/하다는/하다면"은 억제되고, 구두점·공백·마크다운(**`)·
// 화살표·문자열 끝 등 나머지 전부에서 치환된다. 구두점 허용목록은 라운드마다 하나씩
// 빠뜨렸다(콜론→괄호→대시→슬래시… Codex #226 R2~R5) — 목록 대신 반전이 정답(R6).
// 단 닫는 마크업·인용부호는 어절을 끝내지 않는다(Codex #226 R8: "**중요하다**고 평가",
// "“필요하다”는 의견) — 닫는 부호를 건너뛴 다음 문자가 한글 음절이면 활용이 이어지는
// 중이므로 억제한다. 건너뛰는 건 '닫는' 부호뿐: 문장부호(.!?…)를 건너뛰면 붙여 쓴 새
// 문장("다.그리고")이 활용으로 오인되고, 물결(~)을 건너뛰면 늘임("다~그치")이 오인된다.
// 취소선(~~)은 두 글자 대안으로만 건너뛰어 늘임 한 글자(~)와 충돌하지 않는다. 인용
// 마커 [1]·[1, 12]도 어절을 끝내지 않는다(Codex #226 R9: "중요하다[1]고 평가") — 출력
// 포맷이 각주 인용을 요구하므로 완전한 마커(숫자·쉼표만)를 통째로 건너뛴다. 균형 잡힌
// 괄호 삽입구 "(비용도 포함)고"도 마찬가지(R10) — 같은 줄에서 닫히는 완전한 괄호쌍
// (반각·전각, 중첩 없음)만 건너뛴다: 닫히지 않은 여는 괄호는 종전대로 종결 취급이라
// "필요하다(단서항목..." 류가 억제로 새지 않는다. 오판의
// 비대칭: 억제 실패=문어체 잔존(어색), 변환 오발=문법 파괴 — 의심스러우면 억제가 안전.
const CLOSE_MARK = '(?:~~|\\[[0-9]{1,3}(?:\\s*,\\s*[0-9]{1,3})*\\]|\\([^()\\n]*\\)|（[^（）()\\n]*）|[*_`"\'”’」』»〉》)\\]）］])*';
const END_B = '(?!' + CLOSE_MARK + '[가-힣])';
const FORMAL_ENDING_MAPS = [
  { re: new RegExp('가능성이\\s*있다' + END_B, 'g'), to: '수 있어' },
  { re: new RegExp('가능성이\\s*크다' + END_B, 'g'), to: '가능성이 커' },
  { re: new RegExp('관찰\\s*포인트다' + END_B, 'g'), to: '지켜봐야 해' },
  { re: new RegExp('필요하다' + END_B, 'g'), to: '필요해' },
  { re: new RegExp('중요하다' + END_B, 'g'), to: '중요해' },
  { re: new RegExp('중요해진다' + END_B, 'g'), to: '중요해져' },
  { re: new RegExp('확대된다' + END_B, 'g'), to: '커져' },
  { re: new RegExp('확대되면' + END_B, 'g'), to: '커지면' },
  { re: new RegExp('이동한다' + END_B, 'g'), to: '옮겨가' },
  { re: new RegExp('이동하고\\s*있다' + END_B, 'g'), to: '옮겨가고 있어' },
  { re: new RegExp('강화된다' + END_B, 'g'), to: '강해져' },
  { re: new RegExp('상승한다' + END_B, 'g'), to: '올라가' },
  { re: new RegExp('나타난다' + END_B, 'g'), to: '나타나' },
  { re: new RegExp('작용한다' + END_B, 'g'), to: '작용해' },
  // 단순 어미 — 전부 END_B 경계로 통일(마침표·공백 한정이던 구판은 콜론·따옴표 앞에서 샜다).
  // '적이다'가 '이다'보다 먼저 와야 '적'이 안 지워진다.
  { re: new RegExp('([가-힣])적이다' + END_B, 'g'), to: '$1적이야' },
  { re: new RegExp('([가-힣])\\s*있다' + END_B, 'g'), to: '$1 있어' },
  { re: new RegExp('([가-힣])\\s*된다' + END_B, 'g'), to: '$1 돼' },
  { re: new RegExp('([가-힣])\\s*한다' + END_B, 'g'), to: '$1 해' },
  // 명사+이다 반말은 종성이 정한다 — 받침 있으면 '이야'(핵심이야), 없으면 '야'(기회야).
  // 일괄 '야'는 받침 명사를("다행야"), 일괄 '이야'는 모음 명사를("기회이야") 깬다(Codex #226).
  { re: new RegExp('([가-힣])\\s*이다' + END_B, 'g'), to: (m, p1) => p1 + copulaBanmal(p1) },
];

// 한글 음절의 종성 유무 — (코드 - 0xAC00) % 28 === 0 이면 받침 없음
function copulaBanmal(syllable) {
  return (syllable.charCodeAt(0) - 0xac00) % 28 === 0 ? '야' : '이야';
}

export function softenFormalTone(text) {
...
function formatCardsForPrompt(cards = []) {
  return cards.slice(0, 5).map((c, i) => {
    const quotes = Array.isArray(c.fact_sources)
      ? c.fact_sources.filter((s) => s && s.source_quote).slice(0, 2)
      : [];
    const parts = [
      `[${i + 1}] ${c.T || "(제목 없음)"}`,
      c.d ? `   날짜: ${c.d}` : null,
      c.r ? `   지역: ${c.r}` : null,
      c.fact ? `   사실: ${String(c.fact).slice(0, 350)}` : null,
      c.sub ? `   요약: ${c.sub}` : null,
      c.g ? `   의미: ${String(c.g).slice(0, 250)}` : null,
      ...quotes.map((s) => `   원문인용(${s.source_name || "출처"}): "${String(s.source_quote).slice(0, 180)}"`),
      c.src ? `   출처: ${c.src}` : null,
    ].filter(Boolean);
    return parts.join("\n");
  }).join("\n\n");
...
- 근거 카드의 사실을 쓴 문장 끝에는 그 카드 번호를 [1] 또는 [1, 2] 형태로 붙여. 목록에 없는 번호는 절대 쓰지 마.`;
...
export async function synthesizeChatAnswer({ message, intent, cards = [], newsMeta = null, scope = {}, watchTerms = [] }) {
  ...
  const watchNote = Array.isArray(watchTerms) && watchTerms.length
    ? `\n[사용자 내워치 목록] ${watchTerms.slice(0, 12).join(", ")} — 질문이 '내 워치/관심 목록'을 언급하면 이 목록 기준으로 답해.`
    : "";
  const userPrompt = `사용자 질문: ${message}${regionNote}${watchNote}${dateNote}\n\n근거 카드:\n${cardBlock}\n\n${instruction}`;
...
// ============================================================================
// 검색어 확장 — lexical 리트리벌이 약할 때 1회 호출하는 저비용 확장기
// ============================================================================

const SYSTEM_QUERY_EXPAND = `너는 배터리·ESS·EV 산업 뉴스 검색어 확장기다.
사용자 질문을 내부 뉴스카드 검색에 쓸 키워드로 확장해라.
규칙:
- 한국어·영어 동의어, 업계 전문용어, 관련 기업·기술명을 섞어 3~6개.
- 각 키워드는 2~20자. 문장·설명 금지.
- 출력은 순수 JSON 배열 하나만: ["급속충전", "fast charging", "충전속도"]`;

export async function expandSearchQuery(message) {
  const result = await callLLM({
    system: SYSTEM_QUERY_EXPAND,
    user: `질문: ${String(message || "").slice(0, 200)}`,
    maxTokens: 120,
    temperature: 0.2,
    timeoutMs: 6000,
  });
  if (!result?.text) return { keywords: null, error: result?.error || "no-text", latencyMs: result?.latencyMs || 0 };
  try {
    ...
    const keywords = Array.isArray(arr)
      ? arr.map((k) => String(k || "").trim()).filter((k) => k.length >= 2 && k.length <= 20).slice(0, 6)
      : null;
    return { keywords: keywords && keywords.length ? keywords : null, error: null, latencyMs: result.latencyMs || 0 };
  } catch {
    return { keywords: null, error: "parse-failed", latencyMs: result.latencyMs || 0 };
  }
}
...
export function detectInjectionLeak(text) {
