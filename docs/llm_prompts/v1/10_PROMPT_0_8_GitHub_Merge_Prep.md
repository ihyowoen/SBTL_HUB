...
The legacy body remains subordinate. Any conflicting public-baseline, replace-all-only, fixed-document, or fallback clause is superseded by this canonical entry point and the registered addendum.
...
`public/data/cards.json` is never a merge baseline or fallback source of truth. It is regenerated only after the updated full passes validation.
...
If any assembled instruction still treats `public/data/cards.json` as the merge baseline, stop with:
...
→ apply update
→ apply related_add
→ validate declared diff and related preservation
→ write data/cards.full.json
→ regenerate public/data/cards.json
...
- `public/data/cards.json` is the merge source of truth;
...
- the canonical full and lean projection both pass validation;
