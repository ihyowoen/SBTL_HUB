...
function detectFeedFilter(msg) {
  ...
  // 클라이언트 피드 필터는 단일 축(지역|워치|시그널 중 하나)이라 복합 면을 다 적용할 수 없다.
  // 겹치면 클라 적용 우선순위(지역 > 워치 > 시그널)와 같은 순서로 한 축만 남겨 —
  // 응답 라벨과 실제 화면이 항상 일치하게. 기간·검색어는 별도 상태라 자유롭게 결합.
  // (다축 결합은 피드 필터 UI 확장과 함께 R8b에서.)
  if (facets.region) { delete facets.watch; delete facets.signal; }
  else if (facets.watch) delete facets.signal;
