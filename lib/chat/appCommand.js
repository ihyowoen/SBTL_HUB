...
function detectFeedFilter(msg) {
  ...
  // 지역/워치가 있으면 시그널 면을 버려 — 응답 라벨("중국으로 맞춰서")과 실제 화면이 일치하게.
  // (시그널 단독 "TOP만 보여줘"는 그대로 동작. 다축 결합은 피드 필터 UI 확장과 함께 R8b에서.)
  if ((facets.region || facets.watch) && facets.signal) delete facets.signal;
