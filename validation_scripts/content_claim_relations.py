...
def _korean_local_period_descriptions(clause_text: str):
    ...
    if len(periods) < 2:
        return ()
    subject_match=re.match(r'^' + _KR_SUBJECT,clause_text)
    ...
    for index,(start,end,period) in enumerate(periods):
        ...
        match=_KR_LOCAL_DESCRIPTION.fullmatch(local)
        if not match:
            continue
        out.append((
...
def korean_period_relations(text: str) -> Counter:
    ...
    for clause in clauses(text):
        ...
        for local_key in _korean_local_period_descriptions(clause.text):
            out[local_key]+=1
...
_KR_OBJECT_BEFORE_PERIOD = re.compile(
    ...
    r"(?:\s+[가-힣A-Za-z][가-힣A-Za-z0-9&_-]*)*)(?:을|를)"
    r"(?:\s+(?:주로|대체로|대부분|통상|보통))?\s*$"
)
...
_PROPER = (r"(?:[A-Z][A-Za-z0-9&._-]+|[0-9]+[A-Za-z][A-Za-z0-9&._-]*)"
           r"(?:(?:'s|’s)\s+[A-Za-z][A-Za-z0-9&._-]+)?"
...
def _canonical_metric_observation(operator: str, raw: str) -> str:
    phrase=((operator or '').strip() + ' ' + str(raw or '').strip()).strip()
    observations=atoms.temporal_period_observations(phrase)
    if observations:
        return observations[0][2]
    return _canonical_metric_period(raw)
...
def _strip_subject_preverb_period(text: str):
    ...
    aux_pattern=(
        ...
        r"can|could|may|might|will|would|must|should|shall|not)"
    )
    for start,end,period in atoms.temporal_period_observations(value):
        if start < subject_match.end():
            continue
        between_raw=value[subject_match.end():start]
        paren_bridge=re.fullmatch(
            r"(?P<between>(?:(?:" + aux_pattern + r")\s+)*)\(\s*",
            between_raw,
            re.I,
        )
        paren_wrapped=(
            paren_bridge is not None
            and re.match(r'^\s*\)',value[end:]) is not None
        )
        if paren_wrapped:
            between=(paren_bridge['between'] or '').strip()
        else:
            between=re.sub(r",\s*$","",between_raw.strip()).strip()
        ...
        if paren_wrapped:
            ...
            suffix=re.sub(r"^\s*\)"," ",value[end:],count=1)
        else:
            prefix=re.sub(r",\s*$"," ",value[:start])
...
def _local_dated_arguments(tail: str):
    """Return copular complement terms bound to each explicit local period."""
    periods=atoms.temporal_period_observations(tail)
    if not periods:
        return ()
    out=[]
    previous_end=0
    connector_re=r"\b(?:and|or|but|while|whereas)\b"
    for index,(start,end,period) in enumerate(periods):
        ...
        if preposed:
            ...
            local=before
            boundaries=list(re.finditer(connector_re,local,flags=re.I))
            if boundaries:
                local=local[boundaries[-1].end():]
        local=re.sub(
            r'^\s*(?:(?:and|or|but|while|whereas)\s+|,)','',local,flags=re.I
        )
        local=re.sub(
            r'\s*(?:(?:and|or|but|while|whereas)|,)\s*$','',local,flags=re.I
        )
...
def _active_local_dated_pairs(tail: str, initial_subject: str,
    ...
    for index,(start,end,period) in enumerate(periods):
        ...
        period_preposed_to_next=bool(
            re.search(r"\b(?:and|or|but|while|whereas)\s*$",raw_local,re.I)
        )
        after=tail[end:next_start]
        after_clean=re.sub(
            r'^\s*(?:,|and|or|but|while|whereas)?\s*','',after,flags=re.I
        )
        ...
        connector_prefix=re.match(
            r'^\s*(?:and|or|but|while|whereas|,)\s+',raw_local,re.I
        )
        local_source=raw_local
        finite_boundary=None
        for boundary in re.finditer(
            r"\b(?:and|or|but|while|whereas)\b",raw_local,re.I
        ):
            candidate=raw_local[boundary.end():].strip(" ,")
            candidate_terms=ordered_terms(candidate)
            if candidate_terms and _finite_coord_token(candidate_terms[0]):
                finite_boundary=boundary
        if finite_boundary is not None:
            local_source=raw_local[finite_boundary.end():]
            connector_prefix=True

        local=re.sub(
            r'^\s*(?:and|or|but|while|whereas|,)\s*','',local_source,flags=re.I
        ).strip()
        local=re.sub(
            r'\s*(?:and|or|but|while|whereas|,)\s*$','',local,flags=re.I
        ).strip()
...
def _preposed_action_terms(tail: str):
    ...
    connectors=list(re.finditer(
        r"\b(?:and|or|but|while|whereas)\b",before,flags=re.I
    ))
    if connectors:
...
def _passive_local_dated_objects(passive):
    ...
    for index,(start,end,period) in enumerate(periods):
        ...
        local=re.sub(
            r'^\s*(?:,\s*)?(?:(?:and|or|but|while|whereas)\s+)?',
            '',local,flags=re.I,
        ).strip()
        local=re.sub(
            r'\s*(?:(?:and|or|but|while|whereas)\s*)?,?\s*$',
            '',local,flags=re.I,
        ).strip()
...
def _masked_passive_period_objects(raw: str, passive):
    ...
    for start,end,period in periods:
        if object_start <= start <= object_end:
            local=clean_local(raw[object_previous:start])
        ...
        if start >= tail_start:
            local=clean_local(raw[tail_previous:start])
            object_terms=ordered_terms(_mask_periods(local)) if local else ()
            out.append((object_terms or base_object,period))
            tail_previous=end
            continue
    ...
    if direct:
        ...
        if local_pairs:
            if preposed_periods:
                ...
                pieces=re.split(r"\b(?:and|or|but|while|whereas)\b",before_first,flags=re.I)
...
_COORDINATED_METRIC_PERIOD = re.compile(
    r'(?:^|(?:and|,))\s*(?P<operator>' + _METRIC_TEMPORAL_OPERATOR + r')\s+'
        ...
        for quantity in atoms.quantitative_observations(clause.text):
            ...
            if match:
                ...
                if carry and context is not None:
                    ...
                    period=_canonical_metric_observation(
                        carry.groupdict().get('operator') or '',carry['period']
                    )
