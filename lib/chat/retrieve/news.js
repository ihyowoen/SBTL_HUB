...
function entityPrefer(cards, ents) {
  ...
  const hit = (c) => {
    ...
    return ents.some((e) => entityTextMatch(hay, e));
  };
...
function selectNewsCards(cards, query, scope = {}, limit = 4) {
  ...
  // 엔티티(워치) 후보가 있으면 캡 없이 해당 날짜/지역의 전체 풀을 가져온다 —
  // 사전 캡은 시그널 순 절단이라 하위 랭크의 워치 카드가 재정렬 전에 탈락한다.
  // 단일 날짜 풀은 수십 장 수준이라 전량 처리 비용은 무시 가능.
  const datedPool = ents.length ? Number.MAX_SAFE_INTEGER : limit;
