...
function parseOps(manifest){ const o=manifest.operations; if(!o||typeof o!=="object"||Array.isArray(o)) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID","operations object required"); const adds=strArr(o.add??[],"operations.add"), updates=strArr(o.update??[],"operations.update"); const migrations=o.id_migration??[]; if(!Array.isArray(migrations)) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID","operations.id_migration must be an array"); const oldIds=[],newIds=[],map=new Map(); for(const [i,x] of migrations.entries()){ if(!x||typeof x!=="object"||Array.isArray(x)) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID",`id_migration[${i}] must be object`); const oldId=str(x.old_id,`id_migration[${i}].old_id`), newId=str(x.new_id,`id_migration[${i}].new_id`); str(x.reason,`id_migration[${i}].reason`); if(oldId===newId||map.has(oldId)) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID",`invalid/duplicate migration ${oldId}`); map.set(oldId,newId); oldIds.push(oldId);newIds.push(newId); }
 if(!uniq(newIds)) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID","duplicate migration new_id"); const sets=[adds,updates,oldIds,newIds]; for(let i=0;i<sets.length;i++) for(let j=i+1;j<sets.length;j++){const b=new Set(sets[j]); if(sets[i].some(x=>b.has(x))) fail("BLOCKED_MANUAL_DIRECT_ADD_INVALID","add/update/id_migration identities must be disjoint");} return {adds,updates,migrations,oldIds,newIds,migrationMap:map}; }
...
function validateV2Editorial(m,ops){
 ...
 const e=m.editorial_attestation; if(!e||typeof e!=="object"||e.policy_version!=="EMBEDDED_NEWS_VALUE_SELECTION_V4") fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL","V2 editorial policy attestation missing/invalid");
 ...
 for(const [i,a] of adds.entries()){
   if(!a||typeof a!=="object") fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL",`addition attestation ${i} invalid`); const id=str(a.id,`addition[${i}].id`); if(addMap.has(id)) fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL",`duplicate addition attestation ${id}`); addMap.set(id,a);
 ...
 for(const [i,u] of ups.entries()){
   if(!u||typeof u!=="object") fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL",`update attestation ${i} invalid`); const id=str(u.id,`update[${i}].id`); if(upMap.has(id)) fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL",`duplicate update attestation ${id}`); upMap.set(id,u); if(!["reinforcement","correction","evidence_update","content_correction"].includes(u.change_type)) fail("BLOCKED_MANUAL_DIRECT_ADD_EDITORIAL",`${id}: invalid change_type`); str(u.reason,`${id}.reason`);str(u.evidence_review_summary,`${id}.evidence_review_summary`);
 }
