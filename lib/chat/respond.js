...
function buildNextContext({ parsed, retrieval, cards, answer, priorContext }) {
  ...
  // last_turn.cards: raw card format (T/sub/g/r/s/src/d/k) — resolveContext/
  // synthesizeCardAnalysis/retrieve prior_cards fallback이 raw 필드를 읽기 때문.
  // UI 표시는 response.cards (toCardView'd)를 사용하고, next_context는 protocol
  // 용이므로 frontend가 그대로 echo 백한다 (D1 해결: URL 문자열 아닌 object 배열).
  const last_turn = {
    action,
    topic,
    answer_text: answer || "",
