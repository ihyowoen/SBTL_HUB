...
function latestCards(cards, limit = 3, region = null, targetDate = null) {
  const pool = region ? cards.filter((c) => c.r === region || (region === "US" && c.r === "NA")) : cards;
...
function lexicalSearchCards(cards, query, limit = 5, scope = {}) {
  ...
  return cards
    .map((c) => {
      ...
      if (scope?.region && (c.r === scope.region || (scope.region === "US" && c.r === "NA"))) score += 8;
      if (scope?.date && c.d && fmtDate(c.d) === fmtDate(scope.date)) score += 6;
      if (scope?.entity) {
