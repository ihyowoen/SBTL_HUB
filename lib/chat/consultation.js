...
function validateRedTeamJson(parsed, stage1Facts = [], stage2 = null) {
  ...
  const FORBIDDEN_META_PATTERNS = [
    ...
    /(?:^|[^0-9])[123]\s*차(?!\s*[0-9])/, // 1차, 2차, 3차 (한글 뒤 \b 미사용)
    /\bfacts?\b/i,                    // facts, fact (영문 단독 사용)
    /\binterpretation\b/i,            // interpretation
    /\bkey_?tension\b/i,              // key_tension, keytension
    /\bpremature(_interpretation)?s?\b/i, // premature, premature_interpretations
    /\bunverified(_premise)?s?\b/i,   // unverified, unverified_premises
    /\bcounter_?scenario\b/i,         // counter_scenario
    /\bcheckpoints?\b/i,              // checkpoints
    /\bf\d+\b/,                       // f1, f2, f3 ... fact id
