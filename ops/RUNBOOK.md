...
| F16 | 노출 필드를 정규식 substring 삭제로 편집해 문장·볼드 마커가 파손된 채 사용자 화면에 렌더 | 편집 후 렌더 기준 재검사 부재 | validate2 검사 #14 + 문장 단위 재작성 원칙 |
...
- **도구 상한 실측(2026-09-09): 한 세션의 WebSearch 는 200회에서 강제 차단된다.** 요구량 379~381 의 약 53% 다. 예산 확대는 상한(`CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`) 조정 또는 세션 분할이지 오케스트레이터 재량이 아니다. 이날 원장에 기록된 검색은 **199회(셀 전수 90 + 표적 109)** 이고 도구 카운터는 200 을 찍었다 — 1회는 기록에 없다(어느 검색인지 재구성 불가, 원장 budgetCeiling 에 명시). 당일 명중은 승인 반영 전 153/188.
