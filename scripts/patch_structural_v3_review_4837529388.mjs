...
const files = {
  stageA: "docs/llm_prompts/v1/01_PROMPT_0_1_Stage_A.md",
  override: "docs/llm_prompts/v1/01A_PROMPT_0_1S_Structural_Value_Override.md",
  stageB: "docs/llm_prompts/v1/02_PROMPT_0_2_Stage_B_r0.md",
  finalQc: "docs/llm_prompts/v1/09_PROMPT_0_7_Final_QC.md",
  relatedValidator: "validation_scripts/related_lifecycle_check.py",
  tests: "validation_scripts/tests/test_workflow_contracts.py",
  validation: "docs/validation/STRUCTURAL_NEWS_VALUE_V3_VALIDATION_20260802.md",
};
const selfPath = "scripts/patch_structural_v3_review_4837529388.mjs";
const workflowPath = ".github/workflows/patch-structural-v3-review-4837529388.yml";
...
Stage A remains no-fetch. Rescue means bounded-question capture and preservation, not external search.`,
    "01A supported review partitions",
  );
  text = replaceOnce(
    text,
"- `review_pool_repromotion_precondition`",
"- `review_pool_subtype`\n- `review_pool_repromotion_precondition`",
    "01A review subtype ledger",
  );
  text = replaceOnce(
    text,
...
"- a mandatory structural domain is zero without recheck and explanation;\n- `structural_signal_review_pool` or `earnings_deep_dive_pool` is emitted as a standalone top-level partition instead of a `candidate_review_pool` subtype;\n- a candidate-review item lacks a valid `review_pool_subtype`;",
    "01A partition blockers",
  );
  return text;
});

update(files.stageA, (text) => {
  text = replaceOnce(
    text,
...
"   - for format-risk items, either a concrete execution anchor or a complete V3 Structural Value Override package with a valid non-execution anchor class, item-specific Stage B evidence targets, and a specific explanation of why execution is not required",
    "Stage A strict format-risk gate",
  );
  text = replaceOnce(
    text,
...
`1. \`candidate_review_pool[]\`
   ...
   - Allowed subtypes are \`general_candidate\`, \`structural_signal_review_pool\`, and \`earnings_deep_dive_pool\`.`,
    "Stage A candidate review definition",
  );
  text = replaceOnce(
    text,
...
"- `execution_anchor_type`\n- `execution_anchor_strength`\n- `structural_value_override_applied`\n- `anchor_classes`\n- `evidence_needed_for_stage_b`\n- `why_execution_event_not_required`\n- `strict_pass_gate_status`",
    "Stage A CSV override columns",
  );
  text = replaceOnce(
    text,
"- `review_pool_partition`\n- `review_pool_partition_reason`",
"- `review_pool_partition`\n- `review_pool_subtype`\n- `review_pool_partition_reason`",
    "Stage A CSV subtype column",
  );
  text = replaceOnce(
    text,
...
"- If `format_risk_tags` is not empty/none, either `execution_anchor_strength` must be `strong` or `moderate`, or `structural_value_override_applied` must be true with at least one valid non-execution `anchor_classes` value, non-empty item-specific `evidence_needed_for_stage_b`, and non-empty specific `why_execution_event_not_required`.",
    "Stage A CSV strict anchor alternatives",
  );
  text = replaceOnce(
    text,
...
"- `promotion_precondition` must not be empty for `candidate_review_pool`.\n- For `candidate_review_pool`, `review_pool_subtype` must be exactly one of `general_candidate`, `structural_signal_review_pool`, or `earnings_deep_dive_pool`.\n- `structural_signal_review_pool` and `earnings_deep_dive_pool` must never appear as top-level `review_pool_partition` values.\n- `recommended_next_action` must not recommend Stage B for `watchlist_context_pool` or `reject_or_support_only_pool`.",
    "Stage A CSV subtype rules",
  );
  return text;
});

update(files.stageB, (text) => {
  text = replaceOnce(
    text,
...
Allowed V3 non-execution anchor classes are: \`policy_regulatory_anchor\`, \`data_financial_anchor\`, \`strategic_behavior_anchor\`, \`technology_commercialization_anchor\`, and \`follow_up_probability_anchor\`. Their use never waives evidence, source-direction, full-schema cardability, or no-anchor-laundering rules.`,
    "Stage B V3 anchor gate",
  );
  return text;
});

update(files.finalQc, (text) => {
  text = replaceOnce(
    text,
...
"- Stage A selector marker is present and accepted. The current marker is `structural_selector_policy_version: STRUCTURAL_NEWS_VALUE_SELECTION_V3`; explicitly authorized legacy markers may be accepted only for a declared one-off recovery run.\n- `strict_pass_gate` or `strict_gate_check` exists for every candidate that originated from `strict_passed_spec[]`\n- every format-risk candidate carries either source-backed `execution_anchor_type` / `execution_anchor_strength`, or a validated V3 Structural Value Override package with a valid non-execution anchor class, item-specific Stage B evidence targets, and a specific explanation of why execution is not required",
    "Final QC V3 required lineage fields",
  );
  text = replaceOnce(
    text,
...
9. docs/OPERATIONS.md
10. docs/POST_ACCEPTANCE_CONTENT_ENRICHMENT_QC.md`,
    "Final QC governance hierarchy",
  );
  return text;
});

update(files.relatedValidator, (text) => {
  text = replaceOnce(
    text,
...
`,
    "Related validator anchor class constant",
  );
  text = replaceOnce(
    text,
...
`    if relation_type == "distinct_follow_up":
        ...
        if anchor_class not in FRESH_FOLLOW_UP_ANCHOR_CLASSES:
            errors.append(
                "distinct_follow_up requires valid fresh_follow_up_anchor_class"
            )
        ...
        if not isinstance(incremental_fact, str) or not incremental_fact.strip():
            errors.append(
                "distinct_follow_up requires incremental_fact_vs_predecessor"
            )
        ...
        if not isinstance(changed_judgment, str) or not changed_judgment.strip():
            errors.append(
                "distinct_follow_up requires changed_judgment_vs_predecessor"
            )
`,
    "Related validator V2 fields",
  );
  return text;
});

update(files.tests, (text) => {
  text = replaceOnce(
    text,
...
`    def test_distinct_follow_up_requires_valid_anchor_class(self):
    ...
    def test_duplicate_cannot_publish(self):
        ...
        self.assertTrue(any("may not use" in error for error in errors))
`,
    "Related validator regression tests",
  );
  return text;
});

update(files.validation, (text) => {
  text = replaceOnce(
    text,
...
"7. structural and earnings-specific review subtypes routed through the supported `candidate_review_pool`;",
    "Validation review subtype wording",
  );
  text = replaceOnce(
    text,
...
"- non-execution follow-ups have a defined Related evidence contract;\n- the production Related validator enforces the V2 anchor-class, incremental-fact, and changed-judgment fields;\n- Stage A review subtypes remain inside the supported candidate partition;\n- Stage B and Final QC accept only fully evidenced V3 non-execution anchor packages;",
    "Validation checklist downstream alignment",
  );
  return text;
});
...
for (const [path, required] of [
  [files.stageA, [
    "review_pool_subtype",
    "structural_value_override_applied",
    "structural_signal_review_pool` and `earnings_deep_dive_pool` are not additional top-level partition arrays",
  ]],
  [files.override, [
    "They are `review_pool_subtype` values inside `candidate_review_pool[]`",
    "a candidate-review item lacks a valid `review_pool_subtype`",
  ]],
  [files.stageB, [
    "Allowed V3 non-execution anchor classes",
    "anchor_evidence_missing",
    "STRUCTURAL_NEWS_VALUE_SELECTION_V3",
  ]],
  [files.finalQc, [
    "Stage A V3 selector safety",
    "validated V3 non-execution anchor package",
    "docs/STRUCTURAL_NEWS_VALUE_SELECTION.md",
  ]],
  [files.relatedValidator, [
    "FRESH_FOLLOW_UP_ANCHOR_CLASSES",
    "incremental_fact_vs_predecessor",
    "changed_judgment_vs_predecessor",
  ]],
  [files.tests, [
    "test_distinct_follow_up_requires_valid_anchor_class",
    "test_distinct_follow_up_requires_incremental_fact",
    "test_distinct_follow_up_requires_changed_judgment",
  ]],
  [files.validation, [
    "Executable alignment completed in this PR",
    "production Related validator enforces",
  ]],
]) {
...
console.log("PASS: review 4837529388 downstream alignment applied; temporary files removed");
