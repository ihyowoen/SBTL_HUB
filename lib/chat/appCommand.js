...
function parsePeriodToken(tok) {
  ...
  const mm = Number(m[2]);
  if (mm < 1 || mm > 12) return { period: null, month: null };
  const now = new Date();
...
export function detectAppCommand(message, aliasGroups = [], opts = {}) {
  ...
  if (briefNowMatch) {
    ...
    const { period: tokPeriod, month } = parsePeriodToken(briefNowMatch[2]);
  ...
  if (weeklyShowMatch) {
    ...
    const { period, month } = parsePeriodToken(weeklyShowMatch[2]);
    if (month) return { type: "weekly_show", period, month };
    return period ? { type: "weekly_show", period } : { type: "weekly_show" };
  }
