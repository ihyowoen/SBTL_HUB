...
function validateRef(errors, ref, path, expectedN) {
  ...
  if (hasOwn(ref, "url") && typeof ref.url !== "string") push(errors, `${path}.url`, "must be a string when present");
}
...
function validateQc(errors, qc, path) {
  ...
  if (hasOwn(qc, "reviewed_at") && !validIsoDate(qc.reviewed_at)) push(errors, `${path}.reviewed_at`, "must be a real YYYY-MM-DD calendar date when present");
  if (hasOwn(qc, "reviewer_role") && !nonEmptyString(qc.reviewer_role)) push(errors, `${path}.reviewer_role`, "must be a non-empty string when present");
  if (hasOwn(qc, "notes") && typeof qc.notes !== "string") push(errors, `${path}.notes`, "must be a string when present");
}
...
export function validateMonthlyBriefItem(item, path = "item") {
  ...
  if (hasOwn(item, "watch")) validateStringArray(errors, item.watch, `${path}.watch`);
  ...
  if (!approval || typeof approval !== "object" || Array.isArray(approval)) {
    ...
    if (hasOwn(approval, "notes") && typeof approval.notes !== "string") push(errors, `${path}.approval.notes`, "must be a string when present");
